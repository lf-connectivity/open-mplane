...
IYangPersist::factory(const std::string& yangModelName) {
...
YangPersist::read() {
  ...
  while (in.good()) {
    ...
    // std::cerr << "YangPersist[" <<  mYangModelName << "] '" << xpath << "' =>
    // '" << value << "'" << std::endl ;
  }
  in.close();
...
//-------------------------------------------------------------------------------------------------------------
std::map<std::string, std::string>
YangPersist::values() const {
  return mValues;
}

//-------------------------------------------------------------------------------------------------------------
void
YangPersist::set(const std::map<std::string, std::string>& values) {
...
//-------------------------------------------------------------------------------------------------------------
void
YangPersist::set(const std::string& xpath, const std::string& value) {
  //    std::cerr << "YangPersist[" <<  mYangModelName << "]::set '" << xpath <<
  //"' = '" << value << "'" << std::endl ;

  // filter out if xpath is not a persistent path
  if (!xpathMatchesPersist(xpath)) {
    //        std::cerr << "YangPersist[" <<  mYangModelName << "]::set - NOT
    // PERSISTENT" << std::endl ;
    return;
  }
  ...
  if (entry == mValues.end()) {
    //        std::cerr << "YangPersist[" <<  mYangModelName << "]::set -
    //        PERSISTENT
    // new" << std::endl ;
    mValues[xpath] = value;
    mDirty = true;
  ...
  //    std::cerr << "YangPersist[" <<  mYangModelName << "]::set - PERSISTENT
  // changed" << std::endl ;
  entry->second = value;
  mDirty = true;
}
...
//-------------------------------------------------------------------------------------------------------------
void
YangPersist::setPersist(const std::set<std::string>& xpaths) {
  mPersistPaths.insert(xpaths.begin(), xpaths.end());
}

//-------------------------------------------------------------------------------------------------------------
void
YangPersist::setPersist(const std::string& xpath) {
...
//-------------------------------------------------------------------------------------------------------------
bool
YangPersist::xpathMatchesPersist(const std::string& xpath) const {
