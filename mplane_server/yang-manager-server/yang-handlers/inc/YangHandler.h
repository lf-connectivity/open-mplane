 ...
 *
 *
 * \details   Registers callbacks with the YANG models so that changes in YANG
 * model parameters can be reflected in the application
 *
 */
...
class YangHandler : public virtual IYangHandler {
 ...
 protected:
  ...
  /**
   * Add this/these paths to a list of paths that will be stored in flash and
   * restored on restart
   */
  virtual void makePersistent(const std::set<std::string>& xpath);

  /**
   * Add this path to a list of paths that will be stored in flash and restored
   * on restart
   */
