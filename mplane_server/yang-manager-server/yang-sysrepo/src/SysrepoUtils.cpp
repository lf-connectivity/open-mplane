...
SysrepoUtils::getKeyParams(
    std::shared_ptr<YangMgrServer> mgr, const std::string& path) {
  return getKeyParams(
      mgr, path, (uint16_t)RadioDatastoreSysrepo::RadDs_E::RADIO_DS_RUNNING);
}

//-------------------------------------------------------------------------------------------------------------
std::map<std::string, std::map<std::string, std::string>>
SysrepoUtils::getKeyValues(
    std::shared_ptr<YangMgrServer> mgr, const std::string& path) {
  return getKeyValues(
      mgr, path, (uint16_t)RadioDatastoreSysrepo::RadDs_E::RADIO_DS_RUNNING);
}
...
SysrepoUtils::getParams(
    std::shared_ptr<YangMgrServer> mgr, const std::string& xpath) {
  return getParams(
      mgr, xpath, (uint16_t)RadioDatastoreSysrepo::RadDs_E::RADIO_DS_RUNNING);
}

//-------------------------------------------------------------------------------------------------------------
std::map<std::string, std::map<std::string, std::shared_ptr<YangParam>>>
SysrepoUtils::getOperationalKeyParams(
    std::shared_ptr<YangMgrServer> mgr, const std::string& path) {
  return getKeyParams(
      mgr,
      path,
      (uint16_t)RadioDatastoreSysrepo::RadDs_E::RADIO_DS_OPERATIONAL);
}

//-------------------------------------------------------------------------------------------------------------
std::map<std::string, std::map<std::string, std::string>>
SysrepoUtils::getOperationalKeyValues(
    std::shared_ptr<YangMgrServer> mgr, const std::string& path) {
  return getKeyValues(
      mgr,
      path,
...
//-------------------------------------------------------------------------------------------------------------
std::map<std::string, std::shared_ptr<YangParam>>
SysrepoUtils::getOperationalParams(
    std::shared_ptr<YangMgrServer> mgr, const std::string& xpath) {
  return getParams(
      mgr,
      xpath,
...
//-------------------------------------------------------------------------------------------------------------
std::map<std::string, std::shared_ptr<YangParam>>
SysrepoUtils::paramsToMap(std::shared_ptr<YangParams> params) {
...
//-------------------------------------------------------------------------------------------------------------
double
SysrepoUtils::paramValDecimal64(
    const std::map<std::string, std::shared_ptr<YangParam>>& values,
    const std::string& key) {
  if (values.find(key) == values.end())
    return 0;
...
//-------------------------------------------------------------------------------------------------------------
std::shared_ptr<sysrepo::Vals>
SysrepoUtils::paramsToVals(std::shared_ptr<YangParams> params) {
  ...
  for (unsigned i = 0; i < numParams; ++i) {
    ...
    switch (param->getType()) {
      ...
      case YangParamType_E::STRING:
        vals->val(i)->set(
            param->name().c_str(), param->getString().c_str(), SR_STRING_T);
        break;
...
//-------------------------------------------------------------------------------------------------------------
std::shared_ptr<YangParams>
SysrepoUtils::valsToParams(std::shared_ptr<sysrepo::Vals> vals) {
...
//-------------------------------------------------------------------------------------------------------------
bool
SysrepoUtils::paramsToValsHolder(
  ...
  for (unsigned i = 0; i < numParams; ++i) {
    ...
    switch (param->getType()) {
      ...
      case YangParamType_E::STRING:
        // std::cerr << " * [" << i << "] '" << param->name() << "' = '" <<
        // param->getString() << "'" << std::endl ;
        vals->val(i)->set(
            param->name().c_str(), param->getString().c_str(), SR_STRING_T);
        break;
...
SysrepoUtils::dumpParamsMap(
  ...
  for (auto entry : values) {
    std::string error;
    std::cerr << entry.first << " [" << entry.second->name()
              << "] = " << YangParamUtils::toString(entry.second, error)
              << std::endl;
  }
}
...
SysrepoUtils::getKeyValues(
    ...
    uint16_t datastoreId) {
  std::map<std::string, std::map<std::string, std::shared_ptr<YangParam>>>
      params(getKeyParams(mgr, path, datastoreId));
  ...
  for (auto entry : params) {
    ...
    for (auto param : entry.second) {
      std::string error;
      values[entry.first][param.first] =
          YangParamUtils::toString(param.second, error);
    }
  }
  return values;
}
