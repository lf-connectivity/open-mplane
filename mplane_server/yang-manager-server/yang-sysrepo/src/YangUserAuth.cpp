...
//-------------------------------------------------------------------------------------------------------------
ssh_keytypes_e
YangUserAuth::strToKeytype(const std::string& typeStr) {
...
YangUserAuth::authParams(
  ...
  //    for (auto entry : args)
  //        std::cerr << "ARGS: " << entry.first << " = " <<
  // YangParamUtils::toString(entry.second, error) << std::endl ;
  ...
  if (args.find("password") != args.end()) {
    ...
    if (args.find("public-key") != args.end()) {
      // need to look through the list of available key algorithms for any we
      // support (only RSA at the moment!)
      ...
      for (unsigned i = 0; i < callList->getNumParams(); ++i) {
        ...
        std::shared_ptr<YangParam> pkey(
            YangParamUtils::find(callList, pkeyPath));
        if (!pkey)
          continue;

        serverKeys.push_back(ISshSession::PublicKey(
            type, YangParamUtils::toString(pkey, error)));
      }
    }
