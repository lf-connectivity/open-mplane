...
//-------------------------------------------------------------------------------------------------------------
int
HandlerCallback::checkPseudoReadOnly(
    sysrepo::S_Session sess,
    const char* module_name,
  ...
  // look at each change and see if any of the values' paths match the read-only
  // set
  try {
    ...
    while (auto change = sess->get_change_next(it)) {
      ...
      if (!valuePath.empty()) {
        //                std::cerr << "Checking CHANGE on " <<
        // valuePath << std::endl ;
        for (auto path : readOnlyPaths) {
          //                    std::cerr << " + " << path << std::endl
          //;

          // if the start of this value's xpath matches any of the read-only set
          // then negate the change
          if (startsWith(valuePath, path)) {
            //                        std::cerr << " + + MATCH: Reject change
            //                        (Read Only)"
            //<< std::endl ;
            mParent->logInfo(
                "Rejected change (Read Only) - '" + valuePath + "'");
...
//-------------------------------------------------------------------------------------------------------------
int
HandlerCallback::handleChangeEvent(
    sysrepo::S_Session sess,
    const char* module_name,
    const std::string& xpathString) {
    ...
    while (auto change = sess->get_change_next(it)) {
      ...
      if (param) {
        ...
        if (!YangParamUtils::toString(param, error).empty()) {
          // If operation is not delete, then add the params to the modified
          // list
...
//-------------------------------------------------------------------------------------------------------------
int
HandlerCallback::handleDoneEvent(
    sysrepo::S_Session sess,
    const char* module_name,
    const std::string& xpathString) {
  try {
      ...
      switch (change->oper()) {
        ...
        case SR_OP_DELETED:
          if (nullptr != change->old_val()) {
            //                    std::cout << "DELETED: ";
            //                    std::cout <<
            // change->old_val()->to_string();,
            mParent->paramDelete(
                xpathString,
                std::make_shared<YangParamSysrepo>(change->old_val()));
          }
          break;
        case SR_OP_MODIFIED:
          if (nullptr != change->old_val() && nullptr != change->new_val()) {
            ...
            //                    std::cout << "old value ";
            //                    std::cout <<
            // change->old_val()->to_string();                     std::cout <<
            // "new value ";
            //                    std::cout <<
            // change->new_val()->to_string();
            mParent->valueChange(
