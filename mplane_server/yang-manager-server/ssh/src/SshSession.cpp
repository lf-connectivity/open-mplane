...
void
keyHashPrint(const std::string& msg, ssh_key pubkey) {
  // convert key to hash
...
//-------------------------------------------------------------------------------------------------------------
std::string
SshSession::user() const {
  return mUser;
}

//-------------------------------------------------------------------------------------------------------------
std::string
SshSession::host() const {
  return mHost;
}

//-------------------------------------------------------------------------------------------------------------
void
SshSession::setDebug(unsigned level) {
...
//-------------------------------------------------------------------------------------------------------------
void
SshSession::setPort(unsigned port) {
  if (mConnected) {
...
//-------------------------------------------------------------------------------------------------------------
void
SshSession::setHost(const std::string& host) {
  mHost = host;
...
//-------------------------------------------------------------------------------------------------------------
void
SshSession::setUser(const std::string& user) {
  mUser = user;
...
//-------------------------------------------------------------------------------------------------------------
void
SshSession::setHostKeyTypes(const std::string& typesList) {
...
//-------------------------------------------------------------------------------------------------------------
void
SshSession::setHostKeys(const std::vector<ISshSession::PublicKey>& keys) {
...
//-------------------------------------------------------------------------------------------------------------
bool
SshSession::isConnected() const {
  return mConnected;
}

//-------------------------------------------------------------------------------------------------------------
bool
SshSession::authenticatePassword(const std::string& password) {
  if (!mConnected) {
...
//-------------------------------------------------------------------------------------------------------------
bool
SshSession::authenticateKey(const std::string& privkey) {
  if (!mConnected) {
    setError("Cannot authenticate unconnected SSH session");
    return false;
  }
  ...
  ssh_key key(nullptr);
  int rc =
      ssh_pki_import_privkey_base64(privkey.c_str(), NULL, NULL, NULL, &key);
  if (rc != SSH_OK) {
    //        setError(ssh_get_error(mSession)) ;
...
//-------------------------------------------------------------------------------------------------------------
bool
SshSession::isAuthenticated() const {
...
//-------------------------------------------------------------------------------------------------------------
void
SshSession::setError(const std::string& error) {
