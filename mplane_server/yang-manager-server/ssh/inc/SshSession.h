 ...
 *
 *
 * \details   Simplified libssh C++ wrapper revealing only the API we need for
 * ORAN sftp transfer
 *
 *             Connection sequence to create an ssh session as "user" on machine
 * "host" is:
 *
 *             SshSession ssh ;
 *             ssh.setHost(host) ;
...
public:
  ...
  */
 virtual void setHostKeys(
     const std::vector<ISshSession::PublicKey>& keys) override;
