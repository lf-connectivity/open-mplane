...
public:
 ...
 typedef enum { LOCKED, NOT_LOCKED } LockStatus_T;

 typedef enum { PARENT, NOK, OK, DISABLED } SourceState_T;
  ...
  * communication paths traversed between the local clock and the grand-master
  * clock.
  */
...
public:
 ...
 typedef enum { MAC_FORWARDABLE, MAC_NONFORWARDABLE } MulticastMACAddr_T;
...
public:
  ...
  */
 using LockStatusChangeFunc =
     std::function<void(const LockStatus_T lockStatus)>;
