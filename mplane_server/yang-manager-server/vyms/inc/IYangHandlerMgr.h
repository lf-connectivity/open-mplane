 ...
 *
 *
 * \details   There will be a singleton of this class which instantiates all of
 * the required handlers for the specific application Singleton needs to be
 * created for every application in the application-specific directory
 *
 */
...
public:
  ...
  */
 static std::shared_ptr<IYangHandlerMgr> create(
 ...
 /**
  * Run the initialisation of the handlers (can only be done once the rest of
 ...
 virtual bool testCallback(
     const std::string& xpath, std::shared_ptr<DataVariant> value) = 0;
} ;
