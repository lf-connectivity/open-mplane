...
//-------------------------------------------------------------------------------------------------------------
Crypt::Crypt(uint32_t key[4]) : mKey{key[0], key[1], key[2], key[3]} {}
...
//-------------------------------------------------------------------------------------------------------------
bool
Mplane::Crypt::encrypt(uint32_t data[2]) {
...
//-------------------------------------------------------------------------------------------------------------
bool
Mplane::Crypt::decrypt(uint32_t data[2]) {
...
Mplane::Crypt::encrypt(
  ...
  // Need to 1. Add a header to the data (with magic & original length), 2. pad
  // to next 64 bit
  ...
  while (inDataStart != inDataEnd) {
    uint32_t v[2] = {
        ByteTransfer::readWord(inDataStart, inDataEnd),
        ByteTransfer::readWord(inDataStart, inDataEnd)};
...
Mplane::Crypt::decrypt(
  ...
  unsigned dataSize;
  {
    ...
    if (v[0] != CRYPT_MAGIC) {
      std::cerr << "Invalid magic = 0x" << std::hex << v[0] << std::dec
                << std::endl;
      return false;
    }
    if (v[1] > inSize) {
      std::cerr << "Invalid size = 0x" << std::hex << v[1] << std::dec
                << " In size=" << inSize << std::endl;
      return false;
    }
...
void
Crypt::encipher(
    unsigned int num_rounds, uint32_t v[2], uint32_t const key[4]) const {
  unsigned int i;
