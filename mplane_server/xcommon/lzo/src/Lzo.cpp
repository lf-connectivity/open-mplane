...
//-------------------------------------------------------------------------------------------------------------
Lzo::Lzo() : mWorkspace(LZO1X_1_MEM_COMPRESS), mDecompressionMinSize(0) {
  init();
}
...
bool
Mplane::Lzo::compress(
    std::vector<uint8_t>::const_iterator inDataStart,
  ...
  //    std::cerr << "Lzo::compress : insize=" << inDataSize << " estimated
  // outsize=" << outSize << std::endl ;

  // compress
  int rc = lzo1x_1_compress(
      &(*inDataStart), inDataSize, buffer, &outSize, mWorkspace.data());
...
Mplane::Lzo::decompress(
  ...
  //    std::cerr << "Lzo::decompress : insize=" << inDataSize << " estimated
  // outsize=" << outSize << std::endl ;
...
//-------------------------------------------------------------------------------------------------------------
void
Lzo::setBlockSize(unsigned size) {
...
//-------------------------------------------------------------------------------------------------------------
unsigned
Lzo::compressedSize(unsigned uncompressedSize) const {
  return (uncompressedSize + (uncompressedSize / 16) + 64 + 3);
}

//-------------------------------------------------------------------------------------------------------------
unsigned
Lzo::uncompressedSize(unsigned compressedSize) const {
  if (compressedSize > 67)
    compressedSize -= (64 + 3);

  // This is a rubbish estimate (even though it is based on the LZO
  // documentation!) They suggest ( (compressedSize -64 - 3) * 16 / 17 ) but
  // that can give much too low a figure Even doubling the number has lead to me
  // seeing where we've been 30% out. If the size ends up too low then we don't
  // allocate enough work space in the output buffer and end up overflowing the
  // buffer with a fault.
  //
