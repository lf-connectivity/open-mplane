...
CaptureWaveData::CaptureWaveData()
    : mError(""), mDataLen(0), mEof(false), mHeader(), mData() {}
...
//-------------------------------------------------------------------------------------------------------------
bool
CaptureWaveData::addData(
    IWaveFile::WaveDataIter& dataStart, IWaveFile::WaveDataIter dataEnd) {
  //    std::cerr << "append() " << std::distance(dataStart, dataEnd) << " bytes
  //[current buffer " << mData.size() << " bytes]" << std::endl ;
  ...
  if (!mHeader) {
    ...
    // std::cerr << " * data bytes left = " << std::distance(dataStart, dataEnd)
    // << std::endl ;
  ...
  // Need to handle the case where the start of the next waveform is part of
  // this packet
...
//-------------------------------------------------------------------------------------------------------------
unsigned
CaptureWaveData::getDataLength() const {
  return mDataLen;
}

//-------------------------------------------------------------------------------------------------------------
bool
CaptureWaveData::endOfFile() const {
  return mEof;
}

//-------------------------------------------------------------------------------------------------------------
void
CaptureWaveData::reset() {
  mError.clear();
  mDataLen = 0;
  mEof = false;
  mHeader.reset();
...
CaptureWaveData::getData() const {
  return mData;
}

//-------------------------------------------------------------------------------------------------------------
unsigned
CaptureWaveData::getWaveIndex() const {
  if (!mHeader)
    return 0;
...
//-------------------------------------------------------------------------------------------------------------
unsigned
CaptureWaveData::getSampleRateKsps() const {
  if (!mHeader)
    return 0;
...
//-------------------------------------------------------------------------------------------------------------
void
CaptureWaveData::setError(const std::string& error) {
  if (!mError.empty())
    mError += "\n";
  mError += error;
}
