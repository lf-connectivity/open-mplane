...
static const unsigned PLAYBACK_HEAD_LEN{
    Sha1::SHA1_SIZE + UNUSED_SIZE + SAMPLE_RATE_SIZE};
...
PlaybackWaveHeader::PlaybackWaveHeader()
    : WaveFileHeader(), mSha1(Sha1::SHA1_SIZE, 0) {}
...
//-------------------------------------------------------------------------------------------------------------
bool
PlaybackWaveHeader::processHeader(
...
//-------------------------------------------------------------------------------------------------------------
std::vector<uint8_t>
PlaybackWaveHeader::getSha1() const {
  return mSha1;
}

//-------------------------------------------------------------------------------------------------------------
std::string
PlaybackWaveHeader::getSha1String() const {
  return Sha1::hashToString(mSha1);
}

//-------------------------------------------------------------------------------------------------------------
unsigned
Mplane::PlaybackWaveHeader::getHeaderSize() const {
  return PLAYBACK_HEAD_LEN + this->super::getHeaderSize();
}

//-------------------------------------------------------------------------------------------------------------
bool
PlaybackWaveHeader::createHeader(
  ...
  if (!this->super::createHeader(
          data, IWaveFile::WaveType::PLAYBACK, PLAYBACK_HEAD_LEN))
    return false;
