...
protected:
 /**
  * Add a string to the stored error string. If stored error string is not empty
  * then a separator is first appended before adding this new string
  * @param error
  * @return always false - this allows you to set the error and return in one
  * line
  */
 virtual bool
 setError(const std::string& error, const std::string& separator = "\n") {
   if (!_mError.empty())
     _mError += separator;
   _mError += error;
 ...
 /**
  * Add a string to the stored error string. If stored error string is not empty
  * then a newline is first appended before adding this new string.
