 ...
 *
 *
 * \details   Common class that encapsulates IQ data (at the GUI/SCPI end on the
 * PC) and loads encapsulated data (in the embedded application)
 *
 */
...
class PlaybackWave {
 public:
  ...
  /**
   * Encapsulate the raw waveform IQ data in a header and block encode. Saves
   * the data to outData vector
   * @param basename    Base filename (without any mPath)
   * @param inData
   * @param outData
   * @return false on error
   */
  virtual bool encapsulate(
      const std::string& basename,
      const std::vector<uint8_t>& inData,
      std::vector<uint8_t>& outData);

  /**
   * Encapsulate the raw waveform IQ data in a header and block encode. Writes
  ...
  /**
   * Set the file mPath. Used for loading data and determining the playback
  ...
  /**
   * Sets up the object to read encapsulated data from the file mPath specified
   * in setFilepath() method. Checks the header and sets up internal pointers
  ...
  /**
   * Sets up the object to read encapsulated data from the specified file mPath.
   * Checks the header and sets up internal pointers ready to read the first
   * block
   * @param filename        Full mPath to input file
   * @return false on error
   */
  ...
  /**
   * Reads in the next block of data from the encapsulated file. Does any
   * decryption/decompression necessary and saves the raw IQ data in the data
   * vector. Sets the EOF flag at end of data
   * @param data
   * @return false on error
   */
  ...
  /**
   * Checks the file against the supported features. If all file features are
   * supported it will return true
   * @param features
  ...
  /**
   * Process the file to determine the total number of bytes of data (i.e. the
   * file size of the unencrypted/uncompressed file without any headers)
   */
