...
//-------------------------------------------------------------------------------------------------------------
std::shared_ptr<IGenericFeature>
IGenericFeature::factory(
    const std::vector<std::string>& featureNames, std::shared_ptr<IKey> key) {
  std::shared_ptr<IGenericFeature> feature(
      std::make_shared<GenericFeature>(featureNames));
  if (!feature)
    return feature;
...
IGenericFeature::factory(
    ...
    IGenericFeature::GenericFeatureType features) {
  std::shared_ptr<IGenericFeature> feature(
      IGenericFeature::factory(featureNames, key));
  if (!feature)
    return feature;
...
//-------------------------------------------------------------------------------------------------------------
std::string
GenericFeature::featureName(IGenericFeature::GenericFeatureType feature) const {
  auto entry(std::find_if(
      mFeatureNames.begin(),
      mFeatureNames.end(),
      [feature](const std::pair<std::string, unsigned>& p) {
        return static_cast<IGenericFeature::GenericFeatureType>(
                   1 << p.second) == feature;
      }));
...
//-------------------------------------------------------------------------------------------------------------
bool
GenericFeature::enableFeature(IGenericFeature::GenericFeatureType features) {
  // see if it set any invalid bits
  if ((features & mInvalidFeatures) !=
      IGenericFeature::GenericFeatureType::NO_FEATURES)
    return false;
...
//-------------------------------------------------------------------------------------------------------------
bool
GenericFeature::enableFeature(const std::string& featureName) {
...
//-------------------------------------------------------------------------------------------------------------
std::string
GenericFeature::encodeFeatures() const {
  if (!mKey)
    return "";
...
//-------------------------------------------------------------------------------------------------------------
bool
GenericFeature::setFeatures(const std::string& encodedFeatures) {
  if (!mKey)
    return false;
...
//-------------------------------------------------------------------------------------------------------------
IGenericFeature::GenericFeatureType
GenericFeature::supportedFeatures() const {
  return mFeatures;
}

//-------------------------------------------------------------------------------------------------------------
void
GenericFeature::setKey(std::shared_ptr<IKey> key) {
  mKey = key;
}

//-------------------------------------------------------------------------------------------------------------
bool
GenericFeature::isSet() const {
  if (!mKey)
    return false;
...
//-------------------------------------------------------------------------------------------------------------
void
GenericFeature::show(std::ostream& os) const {
  ...
  for (unsigned bit = 0; bit < 32; ++bit) {
    ...
    std::string name(
        featureName(static_cast<IGenericFeature::GenericFeatureType>(mask)));
    if (width < name.size())
  ...
  for (unsigned bit = 0; bit < 32; ++bit) {
    ...
    os << "  " << name << pad << ": " << (bits[bit] ? "Enabled" : "Not enabled")
       << std::endl;
  }
}
...
//-------------------------------------------------------------------------------------------------------------
bool
GenericFeature::checkSupported(const std::string& featureName) const {
  IGenericFeature::GenericFeatureType feature(nameToFeature(featureName));
  if (feature == IGenericFeature::GenericFeatureType::NO_FEATURES)
    return false;
...
GenericFeature::init() {
  for (auto entry : mFeatureNames) {
    mValidFeatures |=
        static_cast<IGenericFeature::GenericFeatureType>(1 << entry.second);
  }
...
//-------------------------------------------------------------------------------------------------------------
IGenericFeature::GenericFeatureType
GenericFeature::nameToFeature(const std::string& featureName) const {
