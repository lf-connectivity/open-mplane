...
public:
   void apply(std::complex<T>* cx) {
     ...
     for (unsigned i = 0; i < Nby2; ++i) {
       ...
       cx[i + Nby2] =
           std::complex<T>(cx[i].real() - tempr, cx[i].imag() - tempi);
...
public:
 ...
 virtual void
 fftshift(std::complex<T>* cx) override {
...
public:
 ...
 virtual void
 fft(std::complex<float>* data) override {
   FftCx<float, N, Inverse>::fft(data);
 }

 virtual void
 fftshift(std::complex<float>* data) override {
...
public:
 ...
 virtual void
 fft(std::complex<double>* data) override {
   FftCx<double, N, Inverse>::fft(data);
 }

 virtual void
 fftshift(std::complex<double>* data) override {
