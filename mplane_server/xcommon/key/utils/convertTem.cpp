...
showHelp() {
  std::cout << ""
               ...
               "\n"
               "Usage:\n"
               "    convertTem [options] -d <output directory> <file> [ "
               "<file> .. ]\n"
               "\n"
               "Options:\n"
               "    -h                         This help\n"
               "    -d <dir>                   Set the output directory to "
               "write file into\n"
               "\n";
...
getOptions(ConvertOptions& options, int argc, char** argv) {
  options.debug = false;
...
main(int argc, char** argv) {
  ...
  for (auto srcfile : args) {
    ...
    // replace existing .art extension with something different to show that the
    // file contents are different. I'll arbitrarily pick .ewf (for Encrypted
    // Waveform File). It shouldn't really matter what it is since it's only the
    // GUI and the embedded SW that will see the filename
    ...
    if (!pbWave.encapsulate(
            basename, data, options.outdir + PATHSEP + basename)) {
