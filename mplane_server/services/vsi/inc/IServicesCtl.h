...
/**
 * Virtual class of miscellaneous services. A miscellaneous service is basically
 * anything not Task or Service based, but needs to be created at
 * Service::start() time.
 *
 * This definition is only created so that all miscellaneous services can be
 * derived from this base class and the pointer to the actual instance stored in
 * the service object.
 */
class IServiceMisc {
 public:
...
class IServicesCtl {
 public:
  ...
  /**
   * Register a new service. Adds this service to the list so that it can be
  ...
  /**
   * Register a new task-based service. Adds this service to the list so that it
   * can be controlled like the others
   * @param service
   * @return true if registered ok
   */
  ...
  /**
   * Callback function which is guaranteed to create one and only one instance
   * of the misc service
   */
  using MiscServiceSingletonCreate =
      std::function<std::shared_ptr<IServiceMisc>()>;

  /**
   * Register a miscellaneous service. In this case it registers the service
   * creation function. This is called once at start up.
  ...
  /**
   * Register a new service. Adds this service to the list *before* the last
   * entry in the list. This is used for "plugin" services so they can add their
   * services to the list in front of the final radio service (or whatever the
   * final control service is)
   * @param service
   * @return true if registered ok
   */
