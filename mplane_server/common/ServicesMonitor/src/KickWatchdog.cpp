...
void
Mplane::KickWatchdog::shutdown() {
  mShutdown = true;
}

int
Mplane::KickWatchdog::run() {
  ...
  if ((fd = open("/dev/watchdog", O_RDWR)) == -1) {
    ...
    // no idea why cppcheck would think this was leaking the fd? If we get here
    // then we've failed to open it in the first place! cppcheck-suppress
    // resourceLeak
    return -1;
  }
