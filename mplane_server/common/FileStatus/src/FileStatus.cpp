...
FileStatus::simpleTracker() {
  return (std::make_shared<FileStatus>());
}

//-------------------------------------------------------------------------------------------------------------
std::shared_ptr<IFileStatus>
IFileStatus::simpleTracker() {
  return FileStatus::simpleTracker();
}

//-------------------------------------------------------------------------------------------------------------
std::shared_ptr<IFileStatus>
FileStatus::simpleOutput(StatusOutputFunc func) {
...
//-------------------------------------------------------------------------------------------------------------
std::shared_ptr<IFileStatus>
IFileStatus::simpleOutput(StatusOutputFunc func) {
...
//-------------------------------------------------------------------------------------------------------------
void
FileStatus::copy(std::shared_ptr<IFileStatus> rhs) {
  ...
  std::shared_ptr<FileStatus> fileStatus(
      std::dynamic_pointer_cast<FileStatus>(rhs));
  if (!fileStatus)
    return;
...
//-------------------------------------------------------------------------------------------------------------
void
FileStatus::setOutputFunction(IFileStatus::StatusOutputFunc func) {
  mOutputFunc = func;
}

//-------------------------------------------------------------------------------------------------------------
void
FileStatus::clearOutputFunction() {
  mOutputFunc = IFileStatus::StatusOutputFunc();
}

//-------------------------------------------------------------------------------------------------------------
void
FileStatus::restart(unsigned num) {
  if (num > 0)
    mNumFiles = num;
...
//-------------------------------------------------------------------------------------------------------------
void
FileStatus::operation(const std::string& op) {
  mOp = op;
}

//-------------------------------------------------------------------------------------------------------------
void
FileStatus::start(const std::string& filename) {
  ++mCurrentFile;
  mFile = filename;
  mFileProgress = 0;
...
//-------------------------------------------------------------------------------------------------------------
void
FileStatus::start(const std::string& op, const std::string& filename) {
  operation(op);
  start(filename);
}
...
//-------------------------------------------------------------------------------------------------------------
void
FileStatus::progress(unsigned progressPercent) {
  if (mCurrentFile < 1)
    return;
...
//-------------------------------------------------------------------------------------------------------------
void
FileStatus::skip(unsigned numFiles) {
  if (numFiles == 0)
    return;
...
//-------------------------------------------------------------------------------------------------------------
void
FileStatus::fail(const std::string& error) {
  mError = error;
  mDone = true;
...
//-------------------------------------------------------------------------------------------------------------
bool
FileStatus::isComplete() const {
  return mDone;
}

//-------------------------------------------------------------------------------------------------------------
std::string
FileStatus::getError() const {
  return mError;
}

//-------------------------------------------------------------------------------------------------------------
unsigned
Mplane::FileStatus::getNumFiles() const {
  return mNumFiles;
}

//-------------------------------------------------------------------------------------------------------------
std::string
FileStatus::getFilename() const {
  return mFile;
}

//-------------------------------------------------------------------------------------------------------------
std::string
FileStatus::getOperation() const {
  return mOp;
}

//-------------------------------------------------------------------------------------------------------------
unsigned
FileStatus::getProgress() const {
  return mFileProgress;
}

//-------------------------------------------------------------------------------------------------------------
unsigned
FileStatus::getTotalProgress() const {
...
//-------------------------------------------------------------------------------------------------------------
std::string
FileStatus::outputPercent(unsigned progress) const {
  // Call the output function with a simple string
...
//-------------------------------------------------------------------------------------------------------------
bool
FileStatus::updateProgress(unsigned progress) {
  if (progress > 100)
    progress = 100;
...
#ifdef DEBUG_FILESTATUS
void
FileStatus::debugShow() {
  std::cerr << "FileStatus: File " << mCurrentFile << "/" << mNumFiles
            << " Op=" << mOp << " Name=" << mFile
            << " Progress=" << mFileProgress << "% Total=" << mTotalProgress
            << "% [Done?=" << mDone << " Error=" << mError << "]" << std::endl;
}
#endif
