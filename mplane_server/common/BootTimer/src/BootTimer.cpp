 ...
 *              NOTE: Object is only expected to be called once at various
 *points in the app, and never at the same time by more than one thread, so we
 *don't need a mutex.
 */
...
// We have a fixed (ish) delay while u-boot starts up the kernel and device tree
...
//-------------------------------------------------------------------------------------------------------------
std::shared_ptr<IBootTimer>
IBootTimer::getInstance() {
...
//-------------------------------------------------------------------------------------------------------------
void
BootTimer::nameComplete(const std::string& name) {
  std::shared_ptr<Benchmark> bench(Benchmark::getInstance());
...
BootTimer::show(std::ostream& os) {
  ...
  os << padLeft("Name", NAME_WIDTH, ' ') << " "
     ...
     << padLeft("Total (s)", TIME_WIDTH, ' ') << " " << std::endl;

  os << padLeft("", NAME_WIDTH, '-') << " " << padLeft("", ELAPSED_WIDTH, '-')
     << " " << padLeft("", TIME_WIDTH, '-') << " " << std::endl;
  ...
  for (auto timing : times) {
    ...
    os << padLeft(timing.second, NAME_WIDTH, ' ') << " "
       ...
       << padLeft(ss2.str(), TIME_WIDTH, ' ') << " " << std::endl;
  }
}

//-------------------------------------------------------------------------------------------------------------
float
BootTimer::getStartTime() const {
  return mStartTime;
}

//-------------------------------------------------------------------------------------------------------------
float
BootTimer::getTifTime() const {
  return mTimers.at(BOOT_TIF_NAME) + mStartTime;
}

//-------------------------------------------------------------------------------------------------------------
float
BootTimer::getTifElapsed() const {
  return mTimers.at(BOOT_TIF_NAME);
}

//-------------------------------------------------------------------------------------------------------------
float
BootTimer::getAppTime() const {
  return mTimers.at(BOOT_APP_NAME) + mStartTime;
}

//-------------------------------------------------------------------------------------------------------------
float
BootTimer::getAppElapsed() const {
  return mTimers.at(BOOT_APP_NAME);
}

//-------------------------------------------------------------------------------------------------------------
float
BootTimer::getNameTime(const std::string& name) const {
...
//-------------------------------------------------------------------------------------------------------------
float
BootTimer::getNameElapsed(const std::string& name) const {
  auto entry(mTimers.find(name));
  if (entry == mTimers.end())
    return 0.0;
...
//-------------------------------------------------------------------------------------------------------------
bool
BootTimer::registerAppCompleteCallback(AppCompleteCallback cb) {
...
//-------------------------------------------------------------------------------------------------------------
bool
BootTimer::registerTifCompleteCallback(TifCompleteCallback cb) {
...
//-------------------------------------------------------------------------------------------------------------
bool
BootTimer::registerNamedCompleteCallback(
    const std::string& name, NamedCompleteCallback cb) {
  if (mNamedCallbacks.find(name) == mNamedCallbacks.end())
