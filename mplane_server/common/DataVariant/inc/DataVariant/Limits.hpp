...
class ValConvertBase {
 public:
  ...
  static bool
  isVariable(const std::string& valstr) {
  ...
  static std::string
  variableName(const std::string& valstr) {
...
class ValConvert : public ValConvertBase {
 public:
  explicit ValConvert<T>(const std::string& valstr, const std::string& unitsStr)
      : ValConvertBase(unitsStr) {}
  ~ValConvert<T>() {}
  T
  val() const {
    return T();
...
class ValConvert<float> : public ValConvertBase {
 public:
  ValConvert<float>(const std::string& valstr, const std::string& unitsStr)
      : ValConvertBase(unitsStr), mVal(0.0) {
...
class ValConvert<int> : public ValConvertBase {
 public:
  ValConvert<int>(const std::string& valstr, const std::string& unitsStr)
      : ValConvertBase(unitsStr), mVal(0.0) {
...
class ValConvert<unsigned> : public ValConvertBase {
 public:
  ValConvert<unsigned>(const std::string& valstr, const std::string& unitsStr)
      : ValConvertBase(unitsStr), mVal(0.0) {
...
class ValConvert<Frequency> : public ValConvertBase {
 public:
  ValConvert<Frequency>(const std::string& valstr, const std::string& unitsStr)
      : ValConvertBase(unitsStr), mVal(0.0) {
...
class ValConvert<Power> : public ValConvertBase {
 public:
  ValConvert<Power>(const std::string& valstr, const std::string& unitsStr)
      : ValConvertBase(unitsStr), mVal(0.0) {
...
class LimitVal {
 public:
  ...
  void
  set(T val, const std::string& valStr) {
    mVal = val;
    mValStr = valStr;
...
class Limits {
 public:
   ...
   *       min..max/step
   *
   *     Where all but one of the parameters is optional at any one time. So the
   * following are valid:
   *
   *       ..max/step
   *       min../step
   ...
   *
   * @param limitsStr
   * @return true if string is in a valid format and the limits have been set
   * accordingly
   */
  bool
  set(const std::string& limitsStr) {
    ...
    if (mMin.isSet() && mMax.isSet() && !mMin.isVariable() &&
        !mMax.isVariable()) {
   ...
   */
  void
  setUnits(const std::string& unitsStr) {
    mUnitsStr = unitsStr;
  }
   ...
   */
  bool
  check(T val, std::string& error) const {
   ...
   */
  void
  show(std::ostream& os = std::cout) const {
...
template <typename T>
inline std::ostream&
operator<<(std::ostream& out, const Mplane::Limits<T>& limits) {
  std::stringstream ss;
  limits.show(ss);
  out << ss.str();
  return out;
