...
//----------------------------------------------------------------------------------------------------------------------
/**
 * The Mplane::DataVariantList class is used to manage lists of
 * Mplane::DataVariant objects
 */
class DataVariantList {
 public:
  ...
  /**
   * Convert list of variable to string. Any optional variables will be skipped
  ...
  /**
   * Convert list of variable to string. Shows all variables including empty
   * optionals
   * @param separator Optional separator string used between pairs
   * @return string representation of the list of variables
   */
  ...
  /**
   * Convert list of variable to a list of strings. Any optional variables will
   * be skipped if they are empty strings
   * @return list of variable strings
   */
  std::vector<std::string> toStrings();

  /**
   * Convert list of variable to a list of strings. Shows all variables
   * including empty optionals
   * @return list of variable strings
   */
  std::vector<std::string> toStringsFull();

  /**
   * Convert list of variable to a list of variable/value pairs. Any optional
   * variables will be skipped if they are empty strings
   ...
   * @param separator Optional separator string used between pairs
   * @param showValue Optional flag - if set also shows the current variable
   * value
   * @return string containing variable/type pairs
   */
  std::string varlist(
      const std::string& separator = ",", bool showValue = false);
  ...
  /**
   * Create the variables in the list parsing the string to determine the
   * variable name, type, and any default value
  ...
  /**
   * Checks to see if all the variables in the list have been set to some value.
   * Sets an error message otherwise
  ...
  /**
   * Uses the string fieldList as if it were specified in the constructor, and
   * checks to see if this list contains a superset of the list contained in the
   * fieldList
   * @param fieldList
  ...
  /**
   * Uses the string fieldList as if it were specified in the constructor, and
   * checks to see if this list contains a subset of the list contained in the
   * fieldList
   * @param fieldList
  ...
  /**
   * Reset the entries in the list. For scalar DataVariants this clears the
   * value; for vector DataVariants this empties the contents
   */
  void clearValues();

  /**
   * Check the INT/FLOAT type current value against any set limits. For vectors
   * ensures *ALL* entries are within limits
 ...
 private:
  ...
  std::vector<DataVariant*>::const_iterator findIter(
      const std::string& name) const;

 protected:
  std::string errorVar;
  DataVariant mNullData; //<! Used as a sentinel when data of a particular name
                         // is not found
