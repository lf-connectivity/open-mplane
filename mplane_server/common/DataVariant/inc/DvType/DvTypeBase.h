...
class DvTypeBase : public virtual IDvType {
 ...
 protected:
  ...
  template <class T>
  std::string
  formatLimits(const Limits<T>& limits) const {
    std::stringstream ss;
  ...
  /**
   * If value string has been specified with some units, then remove those units
   * and return them. The value is expected to be integer or float
 ...
 private:
  ...
  /**
   * Optional units string. Just used for display purposes. App can access the
   * units string and use it in any way it sees fit
   */
  std::string mUnits;
};
