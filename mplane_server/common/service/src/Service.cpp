...
Mplane::Service::CtlState
Mplane::Service::start() {
#ifdef DEBUG_SERVICE
  std::cerr << "Service[" << getName()
            << "] start() mState=" << getStateName(mState)
  ...
  if (mState == ServiceCreated) {
    ...
    // don't use mControl because there could be a task switch and be set back
    // to Noop
    return Start;
  }
  return Noop;
...
void
Mplane::Service::eventNotify() {
  Conditional::Lock lock(mNotify);
  mNotify.signal();
}

Service::State
Mplane::Service::getState() {
  return mState;
}
...

{
  ...
  if (mMsecTimerPeriod > 5000) {
    logDebugNormal(
        "Service[%s]: Error creating with timer period greater than 5000 msec",
        name);
  }
}
...
Mplane::Service::run() {
  ...
  mControl = Noop;
  // for now just use sleep until we get the timer stak delivery to the
  // conditional working.
  ...
  while (mState != ServiceShutdown) {
    ...
    if (mControl == Noop) {
      if (mState == ServiceRunning) {
        ...
        serviceError =
            service(); // while we are in the running state, do our stuff.
        if (serviceError != 0) {
          mState = ServiceShutdown;
        }
        ...
        // use this to adjust how long we wait, so our service has accurate
        // update rate
    ...
    loopTime =
        mMsecTimerPeriod - execTime; // correct for the service execution time

    // if our service return an error, let's get out straight away.
    if (serviceError == 0 &&
        mNotify.wait(loopTime) == true) // where we signalled, or did we reach
                                        // timeout
    {
...
bool
Mplane::Service::registerObservers() {
  // nothing to do in the base class, must be overridden
  logDebugNormal(
      "Service[%s]: Class registerObservers() method called, this should be "
      "overridden",
      getName());
...
#ifdef DEBUG_SERVICE
  std::cerr << "Service[" << getName()
            << "] destroy mState=" << getStateName(mState)
            << " mControl=" << getControlName(mControl) << std::endl;
#endif
...
#ifdef DEBUG_SERVICE
    std::cerr << "Service[" << getName()
              << "] destroy (call shutdown()) mState=" << getStateName(mState)
              << " mControl=" << getControlName(mControl) << std::endl;
#endif
    shutdown();
    Task::msSleep(1);
...
#ifdef DEBUG_SERVICE
  std::cerr << "Service[" << getName()
            << "] destroy END mState=" << getStateName(mState)
            << " mControl=" << getControlName(mControl) << std::endl;
#endif
}

void
Mplane::Service::doReset() {
  // nothing to do in the base class, must be overridden
  logDebugNormal(
      "Service[%s]: Class doReset() method called, this should be overridden",
      getName());
}
...
Mplane::Service::show() {
  // nothing to do in the base class, must be overridden
  logDebugNormal(
      "Service[%s]: Class show() method called, this should be overridden",
      getName());
}

const char*
Mplane::Service::getStateName(State state) {
...
const char*
Mplane::Service::getControlName(CtlState ctlState) {
  const char* stateName = "Unknown";
