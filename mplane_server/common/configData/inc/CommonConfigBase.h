 ...
 * \brief     Common configuration record base class used by all configuration
 * record implementation
 *
 *
 * \details   Common configuration record base class used by all configuration
 * record implementation
 *
 */
...
class CommonConfigBase : public IConfigBase, public Loggable {
 public:
  ...
  /**
   * Pure virtual interface that must be implemented by the actual configuration
   * record. This method is called early in system initialisation to load and
   * parse the data associated with a specific configuration data type.
   *
  ...
  /**
   * Method set by the class that implements the load() method to indicate that
   * the configuration data has been successfully loaded, it can be changed
   * using the protected setLoaded() method.
   * @return
   */
 ...
 private:
  int mVersion; //>! Holds the configuration data version number, initialised to
                //-1
  std::string mInfo; //>! Holds the configuration data information string
  bool mIsLoaded; //>! Flag used to indicate if the configuration data is
                  // successfully loaded
