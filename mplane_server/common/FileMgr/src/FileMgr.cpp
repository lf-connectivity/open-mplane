...
//-------------------------------------------------------------------------------------------------------------
std::shared_ptr<IFileMgr>
IFileMgr::factory(const std::string& dir) {
...
class FileMgrWriter : public virtual IFileMgrWriter {
 public:
   ...
   */
  virtual bool
  addData(const std::vector<uint8_t>& data) override {
    if (mFd <= 0)
      return false;
    ...
    // std::cerr << "Written " << mTotalWritten << " / " << mFileSize <<
    // std::endl ;
   ...
   */
  virtual void
  setStatusObject(std::shared_ptr<IFileStatus> status) override {
    mFileStatus = status;
  }
...
//-------------------------------------------------------------------------------------------------------------
bool
FileMgr::setDir(const std::string& dir, bool mkdir) {
...
//-------------------------------------------------------------------------------------------------------------
bool
FileMgr::setTempDir(const std::string& dir, bool mkdir, bool clear) {
  // create path if necessary
  if (mkdir) {
    if (!Path::exists(dir)) {
      if (Path::mkpath(dir) != 0) {
...
//-------------------------------------------------------------------------------------------------------------
std::string
FileMgr::getDir() const {
  return mDir;
}

//-------------------------------------------------------------------------------------------------------------
std::string
FileMgr::getTempDir() const {
  return mTempDir;
}
...
//-------------------------------------------------------------------------------------------------------------
bool
FileMgr::delfile(const std::string& name) {
  ...
  if (name.find("*") != std::string::npos) {
    // What I should be able to do here is create a std::regex string and use
    // it, but there's a bug in libstdc++ regex anchor handling (though not in
    // the C lib suprisingly!) so I can't use regex like "^LTE_.*$". Turns out
    // they've known about it since 2012 - see
    // https://gcc.gnu.org/bugzilla/show_bug.cgi?id=53631
    //
    // Basically they've deleted all bugs related to the piss poor regex
    // implementation as being a duplicate of this one and then marked this as
    // being "RESOLVED FIXED" because they can't be arsed to actually implement
    // the required code. Wish we could get away with that sort of nonsense!
    //
    // As such the regex is f**king useless! So, to save us from wantonly
    // deleting everything I've limited support to just matching with the start
    // of a string.
    //
    // I've added all this guff in the comments because I've just p*ssed away
    // around an hour of my life trying to work out what I was doing wrong, only
    // to find out it's the library. Not that I'm at all annoyed about this!
    //
...
//-------------------------------------------------------------------------------------------------------------
bool
FileMgr::del(unsigned fileIndex) {
...
//-------------------------------------------------------------------------------------------------------------
bool
FileMgr::del(const std::string& filename) {
  if (mDir.empty()) {
    setError("Directory not set");
    return false;
  }
...
//-------------------------------------------------------------------------------------------------------------
bool
FileMgr::download(const std::string& url, const std::string& filename) {
...
FileMgr::download(
  ...
  int ret = systemTask->runCmd(
      wgetCmd,
      [status](unsigned progress) { status->progress(progress); },
...
FileMgr::portTransferStart(const std::string& filename, unsigned fileSize) {
  return portTransferStart(filename, fileSize, IFileStatus::nullObject());
}

//-------------------------------------------------------------------------------------------------------------
std::shared_ptr<IFileMgrWriter>
FileMgr::portTransferStart(
    const std::string& filename,
    unsigned fileSize,
    std::shared_ptr<IFileStatus> fileStatus) {
  ...
  std::shared_ptr<IFileMgrWriter> fileWriter(
      std::make_shared<FileMgrWriter>(mTempFilepath, fileSize, fileStatus));
  return fileWriter;
}

//-------------------------------------------------------------------------------------------------------------
bool
FileMgr::portTransferEnd(bool ok) {
  return downloadComplete(!ok, false);
}

//-------------------------------------------------------------------------------------------------------------
unsigned long long
FileMgr::dirFreeSpace() const {
  if (mDir.empty())
    return 0;
...
//-------------------------------------------------------------------------------------------------------------
std::string
FileMgr::indexToFilename(unsigned index) {
  std::vector<std::string> files(listFiles());
...
//-------------------------------------------------------------------------------------------------------------
void
FileMgr::setError(const std::string& error) {
  if (!mError.empty())
    mError.append("\n");
  mError.append(error);
}
...
//-------------------------------------------------------------------------------------------------------------
bool
FileMgr::isFreeFileSpace(const std::string& dir, unsigned requiredSize) {
...
//-------------------------------------------------------------------------------------------------------------
bool
FileMgr::downloadStart(const std::string& filename) {
  if (mDir.empty()) {
    setError("Directory not set");
    return false;
  }
  ...
  if (Path::exists(mTempFilepath)) {
    if (::remove(mTempFilepath.c_str()) != 0) {
      setError(
          std::string("Unable to remove file ") + mTempFilepath + " " +
          strError(errno));
      return false;
    }
  }
  if (Path::exists(mDestFilepath)) {
    if (::remove(mDestFilepath.c_str()) != 0) {
      setError(
          std::string("Unable to remove file ") + mDestFilepath + " " +
          strError(errno));
      return false;
    }
  }
...
//-------------------------------------------------------------------------------------------------------------
bool
FileMgr::downloadComplete(bool error, bool timedOut) {
  if (error) {
    if (timedOut) {
