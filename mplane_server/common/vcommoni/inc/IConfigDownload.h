 ...
 *
 *
 * \details   Interface to class that provides the methods to download config
 * files
 *
 */
...
class IConfigDownload {
 public:
  ...
  virtual bool getAndInstall(
      const std::string& remoteFilename, std::string& failReason) = 0;
  ...
  virtual bool setInstallFileName(
      const std::string& name, std::string& failReason) = 0;
  ...
  /**
   * Adds a path and it's configuration date - used to display the list of cal
   * dates for each path
   */
   ...
   * preparation for installing a cal file via TCP port
   */
  ...
  /**
   * Callback function used to do the cal file transfer via TCP. Returns true if
 ...
 protected:
  ...
  virtual bool setInstallDir(
      const std::string& dir, std::string& failReason) = 0;
