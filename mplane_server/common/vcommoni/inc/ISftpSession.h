...
class ISftpSession {
 public:
   ...
   */
  using SftpFileCallback = std::function<void(
      const std::string& remoteFile,
      const std::string& localFile,
      SftpTransaction transaction,
   ...
   */
  using SftpDataCallback = std::function<void(
      const std::string& remoteFile,
  ...
  /**
   * Start a file upload. Transfer will complete in the background and the
   * "complete" function called when done
   * @return TRUE if file transfer scheduled ok; false otherwise
   */
  virtual bool fileUpload(
      const std::string& remoteFile,
      const std::string& localFile,
      SftpFileCallback complete,
      unsigned timeoutSecs = 0) = 0;

  /**
   * Start a file download. Transfer will complete in the background and the
   * "complete" function called when done
   * @return TRUE if file transfer scheduled ok; false otherwise
   */
  virtual bool fileDownload(
      const std::string& remoteFile,
      const std::string& localFile,
      SftpFileCallback complete,
      unsigned timeoutSecs = 0) = 0;
  ...
  /**
   * Start a file upload from a data string. Transfer will complete in the
   * background and the "complete" function called when done
   * @return TRUE if file transfer scheduled ok; false otherwise
   */
  virtual bool dataUpload(
      const std::string& remoteFile,
      const std::string& data,
      SftpDataCallback complete,
      unsigned timeoutSecs = 0,
      int fileOptions = S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH) = 0;

  /**
   * Start a file download to a data string (data will be provided in callback).
   * Transfer will complete in the background and the "complete" function called
   * when done
   * @return TRUE if file transfer scheduled ok; false otherwise
   */
  virtual bool dataDownload(
      const std::string& remoteFile,
      SftpDataCallback complete,
      unsigned timeoutSecs = 0) = 0;
