 ...
 * inherited from and the run method implemented before the it can be
 * instantiated and the thread created.
 *
 * The task thread will be created by default as a detached thread using the
 * SCHED_FIFO scheduling scheme. Its priority will default to the minimum
 * priority and will have a small stack size of size PTHREAD_STACK_MIN.
 *
...
class Task : public Loggable {
 ...
 protected:
   ...
   * it for execution. This constructor is protected as it must be inherited
   * from in order to be instantiated.
   *
   ...
   * @param suName sub-system name that identifies the group this task belongs
   * to.
   */
  Task(
      int priority = PRIORITY_DEFAULT,
      StackSize stacksize = DEFAULT_STACK,
 ...
 private:
   ...
   * specific data stored in the task before calling the virtual method
 ...
 private:
  ...
  /**
   * Flag set when the tasks thread was cancelled, as opposed to exiting
   * normally from its run method.
   */
  bool mCancelled;
  ...
  /**
   * Keep a copy of the TaskList shared pointer. This ensures that while at
   * least one task is created then the TaskList can't go out of scope and get
   * deleted (which causes a seg fault because we need to use the TaskList to
