 ...
 *
 * \file      Conditional.h
 * \brief     A C++ wrapper class for POSIX conditional variable, with
 * supporting mutex
 *
 *
 * \details   A C++ wrapper class for POSIX conditional variable, with
 * supporting mutex
 *
 */
 ...
 * a mutex that is used to lock access to the conditional data, with various
 * wait and signalling methods that act on the conditional variable.
 *
 ...
 *         while( queue.empty() )
 *         {
 *            mCond.wait();                   // release mutex and wait for
 * conditional
 *         }
...
class Conditional {
 ...
 private:
  ...
  /**
   * @note: Standard copy constructors defined but not implemented as
   * conditionals cannot be copied.
   */
 ...
 public:
  ...
  /**
   * Clears out the conditional ensuring the flag is not true (used to ensure
   * any previous signal to this conditional is ignored before starting to wait
   * for a new signal)
   */
  void clear();
