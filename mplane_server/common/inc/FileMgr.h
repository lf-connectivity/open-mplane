 ...
 *
 *
 * \details   Manages files in a specified directory. Allows for adding more
 * files (downloaded from a url) or deleting existing files.
 *
 */
...
class FileMgr : public virtual IFileMgr {
 public:
  ...
  virtual bool setTempDir(
      const std::string& dir, bool mkdir = true, bool clear = true) override;
  ...
  virtual bool download(
      const std::string& url, const std::string& filename) override;
  ...
  virtual std::shared_ptr<IFileMgrWriter> portTransferStart(
      const std::string& filename, unsigned fileSize) override;
 ...
 protected:
  ...
  /**
   * Is there enough free space in the specified directory to store a file of
   * the specified size?
   * @param dir
   * @param requiredSize
