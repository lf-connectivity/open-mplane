...
class Observer {
 public:
  ...
  virtual ~Observer() {}
  /**
   * The callback method used by the corresponding Mplane::Subject
   * implementation to notify the observer of any change in the subject data.
   * The default implementation here must be overridden by the observer.
   *
   * @param subject
   */
...
/**
 This template class provides an implementation of subject class that can be
 extended to provide subject change notification to any attached
 Mplane::Observer based classes.

 This class uses it's own Mutex to support thread safe access to its Observer
 list.
...
class Subject {
 public:
   ...
   * Constructor
   */
  Subject() : mutex(std::shared_ptr<Mutex>(new Mutex())) {}
  /**
  ...
  /**
   * Method called by a critical observer to attach to the subject. This
   * function is not virtual as it is not expected to be overridden.
   *
   * Critical observers are notified first when being notified via their
   * update() method.
   *
   * Typically, there should only be one critical observer in this design
   * pattern, but no restriction is made in this implementation and the critical
   * observer is placed at the front of the list, thus taking precedence over
   * any other critical observer that might have attached already.
   *
   * @param observer
   */
  void
  attachCritical(Observer<T, D>& observer) {
  ...
  /**
   * Method called by the subject destructor to detach all observers from the
   * subject.
   */
  void
  detachAll() {
  ...
  UINT32
  getNumObservers() { return observers.size(); }

  /**
   * A method used to get the name of the observer
   */
  const char*
  getObsName(UINT32 index) const {
