...
class Power : public ComparisonOperators<Power> {
 ...
 public:
   ...
   * @code
   * Power p1(20.0);
   * logDebugNormal("Power is %f %s", p1.get(Power::dBm),
   * p1.unit2Str(Power::dBm) );
   * @endcode
   *
   ...
   * point value.
   * (the equals = operator would normally be used where no unit scaling is
   * required).
   *
   * @code
   * Power p1;
  ...
  /***
   * Method for setting the power in dBm. This method has been optimised as the
   * fastest means of setting a Power object for dB use (lazily implements the
   * Watts power value setting)
   * @param power
   */
  void set(float power);

  /**
   * Method used to set the power value using a string, useful when setting
   * powers from parsed data.
   * @param power
   * @param unit
   */
  ...
  /**
   * Assignment operator to allow the value of a Power object to be set by
   * another.
   *
   * @code
   * Power p1;
  ...
  /**
   * Operator overload to allow a floating point value to be added to a Power
   * object.
   *
   * @code
   * Power p1;
  ...
  /**
   * Operator overload to allow a signed int to be added to an existing Power
   * object.
   *
   * @code
   * Power p1=21;
  ...
  /**
   * Operator overload to allow a signed integer to be multiplied by a Power
   * object.
   *
   * @code
   * Power p1;
  ...
  /**
   * Operator overload to allow a Power object to be multiplied by an existing
   * power.
   *
   * @code
   * Power p1=20;
  ...
  /**
   * Operator overload to allow a signed integer to be divided by a Power
   * object.
   *
   * @code
   * Power p1;
  ...
  /**
   * Operator overload to allow a Power object to be divided by an existing
   * power.
   *
   * @code
   * Power p1=200;
  ...
  /**
   * Operator overload to allow a signed int to divided by an existing Power
   * object.
   *
   * @code
   * Power p1=200;
  ...
  /**
   * Operator overload to allow a Power object to be subtracted from another
   * Power object.
   *
   * @code
   * Power p1=20.0;
  ...
  /**
   * Operator overload to allow a Power object to be subtracted from an existing
   * Power.
   *
   * @code
   * Power f1=220;
 ...
 private:
  /**
   * Internal utility that updates the power in watts based on the current dB
   * power (and also sets the flag) NOTE: Although this is flagged as const it
   * DOES change the object. It actually sets the power in watts + the
   * corresponding flag. Having these data members mutable and this method as
   * const means that the public interface is logically const for getting the
   * values, while internally the watts power is lazily evaluated because most
   * of the time we don't need it
   */
 ...
 */
inline std::ostream&
operator<<(std::ostream& out, const Mplane::Power& power) {
  out << power.get() << "dB";
  return out;
}
