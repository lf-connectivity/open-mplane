 ...
 *
 *
 * \details   Contains a collection of useful functions that operate on the
 * Frequency interface but don't need any internal data details.
 *
 */
 ...
 * @brief range
 *
 * Utility method that takes a centre frequency and a bandwidth and converts
 * them into a min & max frequency
 *
 * @param centre    Centre frequency
 * @param bandwidth    Bandwidth frequency
 * @return vector of 2 frequencies index 0 = min, index 1 = max
 */
inline const std::vector<Frequency>
range(const Frequency& centre, const Frequency& bandwidth) {
