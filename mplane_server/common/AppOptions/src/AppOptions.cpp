...
// Wrap up the AppOptions class so we can create the singleton (makes the
...
//-------------------------------------------------------------------------------------------------------------
std::shared_ptr<IAppOptionSpec>
Mplane::IAppOptionSpec::factory(
    const std::string& name,
    const DataVariantType::Type& type,
    const std::string& description) {
...
Mplane::IAppOptionSpec::factory(
    ...
    const std::string& defaultValue,
    const std::string& useEnvVar) {
  return std::make_shared<AppOptionSpecFactory>(
      name, type, description, defaultValue, useEnvVar);
}
...
//-------------------------------------------------------------------------------------------------------------
DataVariant&
Mplane::AppOptionSpec::getData() {
  return mDataVariant;
}

//-------------------------------------------------------------------------------------------------------------
std::string
Mplane::AppOptionSpec::getUseEnv() const {
  return mUseEnvVar;
}

//-------------------------------------------------------------------------------------------------------------
std::string
Mplane::AppOptionSpec::getDefault() const {
  return mDefault;
}

//-------------------------------------------------------------------------------------------------------------
std::string
Mplane::AppOptionSpec::getDescription() const {
  return mDescription;
}
...
//-------------------------------------------------------------------------------------------------------------
std::shared_ptr<IAppOptions>
Mplane::IAppOptions::singleton(
    int argc,
    const char** argv,
    const std::string& applicationDescription,
    const std::vector<std::shared_ptr<IAppOptionSpec>>& optionsSpecification) {
  static std::shared_ptr<IAppOptions> instance(new AppOptionsSingleton(
      argc, argv, applicationDescription, optionsSpecification));
  return instance;
}

//-------------------------------------------------------------------------------------------------------------
std::shared_ptr<IAppOptions>
Mplane::IAppOptions::createInstance(
    int argc,
    const char** argv,
    const std::string& applicationDescription,
    const std::vector<std::shared_ptr<IAppOptionSpec>>& optionsSpecification) {
  return AppOptions::singleton(
      argc, argv, applicationDescription, optionsSpecification);
}

//-------------------------------------------------------------------------------------------------------------
std::shared_ptr<IAppOptions>
Mplane::IAppOptions::getInstance() {
  return AppOptions::singleton(
      0, NULL, "", std::vector<std::shared_ptr<IAppOptionSpec>>{});
}
...
//-------------------------------------------------------------------------------------------------------------
const DataVariant
Mplane::AppOptions::getOption(const std::string& name) const {
...
Mplane::AppOptions::AppOptions(
    ...
    const std::string& applicationDescription,
    const std::vector<std::shared_ptr<IAppOptionSpec>>& optionsSpecification)
    : mDescription(applicationDescription), mOptions() {
...
//-------------------------------------------------------------------------------------------------------------
void
Mplane::AppOptions::processOptions(int argc, const char** argv) {
...
//-------------------------------------------------------------------------------------------------------------
void
Mplane::AppOptions::processCmdline(int argc, const char** argv) {
  if (argv == NULL) {
    std::cerr << "AppOptions getInstance() called before createInstance()"
              << std::endl;
    abort();
  }
...
//-------------------------------------------------------------------------------------------------------------
bool
Mplane::AppOptions::handleBuiltins(
    int argc, const char** argv, int& idx, const std::string& name) {
  if (name == "help") {
    // Show help then stop
    ...
    for (auto entry : mOptions) {
      ...
      helpOption(
          ...
          entry.second->getUseEnv(),
          entry.second->getDefault());
    }
...
//-------------------------------------------------------------------------------------------------------------
bool
Mplane::AppOptions::handleOption(
    int argc,
    const char** argv,
    int& idx,
    const std::string& name,
  ...
  switch (dv.getType()) {
    ...
    case DataVariantType::Enum: {
      if (++idx >= argc) {
        std::cerr << "Error: Must specify a value for option '" << name << "'"
                  << std::endl;
        exit(1);
      }
    ...
    case DataVariantType::Bool: {
      ...
      bool notVal(false);
      DataVariant defVal(
          DataVariantType::Bool, "default", optionSpec->getDefault());
      if (defVal.isSet())
    ...
    default: {
      std::cerr << "AppOptions invalid DataVariant type for option '" << name
                << "'" << std::endl;
      abort();
    } break;
  }
...
//-------------------------------------------------------------------------------------------------------------
void
Mplane::AppOptions::processDefaults() {
  for (auto entry : mOptions) {
...
//-------------------------------------------------------------------------------------------------------------
void
Mplane::AppOptions::helpOption(
    const std::string& name,
    const DataVariant& dv,
    const std::string& description,
  ...
  // If this option also has an alternate environment variable setting then show
  // it
  std::string envHelp;
  if (!envVarName.empty()) {
  ...
  // Show string
  std::cout << padLeft("  --" + name + " " + typeStr, 30) << " " << description
            << envHelp << defaultHelp << std::endl;
}
