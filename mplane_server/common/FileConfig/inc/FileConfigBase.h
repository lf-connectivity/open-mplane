...
class FileConfigBase {
 public:
  ...
  /**
   * Clean out any invalid variables. This is only performed if the defaults map
   * has been set. If so, then any variables not specified in the defaults map
   * will be removed
   */
  virtual bool clean();
  ...
  /**
   * Set a file path
   * @return TRUE if ok; FALSE otherwise (use error() method to get error
   * string)
   */
  ...
  /**
   * Get all the values as varname/value pairs. Section varnames will consist
   * of: <section name>/<variable>
   */
 ...
 */
inline bool
operator==(const FileConfigBase& lhs, const FileConfigBase& rhs) {
  return lhs.isEqual(rhs);
}

inline bool
operator==(
    std::shared_ptr<FileConfigBase> lhs, std::shared_ptr<FileConfigBase> rhs) {
  return lhs->isEqual(rhs);
}

inline bool
operator==(std::shared_ptr<FileConfigBase> lhs, const FileConfigBase& rhs) {
  return lhs->isEqual(rhs);
}

inline bool
operator==(const FileConfigBase& lhs, std::shared_ptr<FileConfigBase> rhs) {
  return lhs.isEqual(rhs);
}
