...
//-------------------------------------------------------------------------------------------------------------
void
Benchmark::start(const std::string& name) {
...
//-------------------------------------------------------------------------------------------------------------
std::string
Mplane::Benchmark::elapsed(const std::string& name) {
  Mutex::Lock lock(mMutex);
...
//-------------------------------------------------------------------------------------------------------------
std::string
Benchmark::stop(const std::string& name) {
...
//-------------------------------------------------------------------------------------------------------------
std::string
Mplane::Benchmark::timestamp(void) const {
  timespec tNow;
...
//-------------------------------------------------------------------------------------------------------------
float
Benchmark::elapsedTime(const std::string& name) {
  Mutex::Lock lock(mMutex);
  ...
  unsigned us = (unsigned)(tDiff.tv_nsec / 1000L);
  float elapsedSecs =
      static_cast<float>(tDiff.tv_sec) + (static_cast<float>(us) / 1000000.0);
  return elapsedSecs;
}

//-------------------------------------------------------------------------------------------------------------
long
Benchmark::upTime(void) const {
  struct sysinfo info;
  sysinfo(&info);
  return info.uptime;
}
