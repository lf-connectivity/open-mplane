...
//-------------------------------------------------------------------------------------------------------------
std::string
ConfigDownload::getName() const {
  return mConfigDataName;
}

//-------------------------------------------------------------------------------------------------------------
std::string
ConfigDownload::getTypeName() const {
  if (mTypeName.empty()) {
    mTypeName = getName();
...
//-------------------------------------------------------------------------------------------------------------
bool
ConfigDownload::initialise(std::string& failReason) {
  if (!mFileMgr->setTempDir(TEMPDIR, true, true)) {
    failReason = "Unable to initialise temporary directory " + TEMPDIR;
    return false;
  }

  std::string cfgDataPath(
      IAppOptions::getInstance()->getOption("cfg-data-path").toString());
  if (cfgDataPath.empty()) {
    throw std::runtime_error("The app option 'cfg-data-path' must be set.");
  }

  if (!setInstallDir(cfgDataPath, failReason)) {
    return false;
  }
...
//-------------------------------------------------------------------------------------------------------------
bool
ConfigDownload::getAndInstall(
    const std::string& remoteFilename, std::string& failReason) {
...
//-------------------------------------------------------------------------------------------------------------
bool
ConfigDownload::isRunning() const {
...
//-------------------------------------------------------------------------------------------------------------
bool
ConfigDownload::setRemoteUrl(const std::string& url, std::string& failReason) {
  if (isRunning()) {
...
//-------------------------------------------------------------------------------------------------------------
std::string
ConfigDownload::getInstallFilePath() const {
  return getInstallDir() + "/" + getInstallFileName();
}

//-------------------------------------------------------------------------------------------------------------
std::string
ConfigDownload::getInstallFileName() const {
  return mInstallFileName;
}

//-------------------------------------------------------------------------------------------------------------
void
ConfigDownload::show(std::ostream& os) const {
...
//-------------------------------------------------------------------------------------------------------------
bool
ConfigDownload::testInjectInstallDir(
...
//-------------------------------------------------------------------------------------------------------------
void
ConfigDownload::setStatusObject(std::shared_ptr<IFileStatus> status) {
  mStatusObject = status;
}

//-------------------------------------------------------------------------------------------------------------
void
ConfigDownload::addPath(unsigned path, const std::string& calDate) {
  mPathDates[path] = calDate;
}

//-------------------------------------------------------------------------------------------------------------
std::shared_ptr<IFileMgrWriter>
ConfigDownload::getConfigFileWriter(
    std::shared_ptr<IFileStatus> statusObj,
    unsigned fileSize,
    std::string& failReason) {
  // Start the TCP port transfer using default name. If IFileMgrWriter is null
  // then there's an error
  std::shared_ptr<IFileMgrWriter> fileWriter(
      mFileMgr->portTransferStart(mInstallFileName, fileSize, statusObj));
  if (!fileWriter)
...
//-------------------------------------------------------------------------------------------------------------
void
ConfigDownload::startTcpTransfer(TcpConfigTransfer configTransfer) {
  mTcpTransfer = configTransfer;
}

//-------------------------------------------------------------------------------------------------------------
void
ConfigDownload::endTcpTransfer() {
...
//-------------------------------------------------------------------------------------------------------------
bool
ConfigDownload::setInstallFileName(
    const std::string& name, std::string& failReason) {
  if (isRunning()) {
    failReason.assign("Download currently running");
    return false;
  }
...
//-------------------------------------------------------------------------------------------------------------
bool
ConfigDownload::setInstallDir(const std::string& dir, std::string& failReason) {
  if (isRunning()) {
    failReason.assign("Download currently running");
    return false;
  }
...
ConfigDownload::getStatusObject(void) const {
  return mStatusObject;
}

//-------------------------------------------------------------------------------------------------------------
ConfigDownload::ConfigDownloadStatus
ConfigDownload::getStatus() const {
...
//-------------------------------------------------------------------------------------------------------------
void
ConfigDownload::setStatus(const ConfigDownloadStatus& status) {
  Mutex::Lock lock(mMutex);
  mStatus = status;
}
...
//-------------------------------------------------------------------------------------------------------------
bool
ConfigDownload::downloadFile(std::string& failReason) {
