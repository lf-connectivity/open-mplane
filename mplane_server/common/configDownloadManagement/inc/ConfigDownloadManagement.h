...
class ConfigDownloadManagement : public IConfigDownloadManagement, public Task {
 public:
   ...
   */
  virtual bool getAndInstall(
      const std::string& typeName, std::string& failReason) override;
   ...
   */
  virtual void show(
      const std::string& typeName, std::ostream& os = std::cout) const override;
   ...
   * @return filename
   */
  virtual std::string getInstallFileName(
   ...
   * @return file path
   */
  virtual std::string getInstallFilePath(
      const std::string& typeName) const override;
  ...
  /**
   * Method used to add different calibration download types to the calibration
  ...
  virtual bool addConfigDownloadType(
      std::shared_ptr<IConfigDownload> calDownLoad) override;

  /**
   * Return a list of calibration down load types registered with the
   * calibration download manager.
   * @return
   */
  virtual std::vector<std::shared_ptr<IConfigDownload>>
  getConfigDowloadObjects() override;
 ...
 protected:
  ...
  std::shared_ptr<IConfigDownload> getConfigDownload(
  ...
  virtual bool setRemoteUrlNoMutex(
 ...
 private:
  ...
  // Injected install dir (set when we want to debug and override the default
  // path)
  ...
  typedef std::map<std::string, std::shared_ptr<IConfigDownload>>
      MapConfigDownload;
