...
std::string
envStr(const std::map<std::string, ILoggable::LogDebugLevel>& levels) {
  std::string value;
...
LogList::add(ILoggable* logger) {
  ...
  // reject if already in the list - assumes instance name was created using
  // uniqueInstanceName() method
  ...
  ILoggable::LogDebugLevel groupLevel(
      getGroupLevel(logger->getGroupName(), level));
...
LogList::del(ILoggable* logger) {
  ...
  // NOTE: We don't care about the mUniqueMap map, it will just be overwritten
  // with new data as required, so we don't need to delete anything from it
...
//-------------------------------------------------------------------------------------------------------------
void
LogList::show(std::ostream& os) const {
  Mutex::Lock lock(mMutex);

  os << "-- Loggable Objects List --" << std::endl;

  os << padLeft("Name", NAME_WIDTH) << padLeft("Group", GROUP_WIDTH)
     << padLeft("Level", LEVEL_WIDTH) << std::endl;
  os << padLeft("----", NAME_WIDTH) << padLeft("-----", GROUP_WIDTH)
     << padLeft("-----", LEVEL_WIDTH) << std::endl;
...
//-------------------------------------------------------------------------------------------------------------
void
LogList::setLevel(ILoggable::LogDebugLevel level) {
  Mutex::Lock lock(mMutex);

  // Save the level in the level maps (so that any objects created after this
  // setting will get the correct level)
...
//-------------------------------------------------------------------------------------------------------------
std::string
LogList::uniqueInstanceName(const std::string& name) {
  Mutex::Lock lock(mMutex);
  ...
  std::string uniqueName(
      nameCopy + "-" + std::to_string(mUniqueNames[nameCopy]++));
...
//-------------------------------------------------------------------------------------------------------------
std::string
LogList::validGroupName(const std::string& name) const {
...
//-------------------------------------------------------------------------------------------------------------
void
Mplane::LogList::closeRegistration() {
  Mutex::Lock lock(mMutex);
...
LogList::getGroupLevel(
    const std::string& groupName, ILoggable::LogDebugLevel defaultLevel) {
...
//-------------------------------------------------------------------------------------------------------------
ILoggable::LogDebugLevel
LogList::getInstanceLevel(
    const std::string& instanceName, ILoggable::LogDebugLevel defaultLevel) {
