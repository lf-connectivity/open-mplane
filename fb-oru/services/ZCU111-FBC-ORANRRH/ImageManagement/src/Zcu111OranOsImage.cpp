 ...
 *
 * \file      Zcu111OranOsImage.cpp
 * \brief
 *
 *
 * \details   Special ORAN version that "plays well" with the ORAN software management
 *
 */
...
// Partition names
const std::string PARTITION_BOOT("boot") ;
const std::string PARTITION_IMAGE("kernel") ;

// Names of the links used to reflect NOR contents
const std::string imageLink(PARTITION_IMAGE) ;
const std::string bootLink(PARTITION_BOOT) ;
...
// 75M took 4m21s
const float TIMING_FACTOR( 261.0 / 78317504.0 ) ;

// Use this size (80M) in estimates
const unsigned IMAGE_SIZE(80 * 1024 * 1024) ;
...
//-------------------------------------------------------------------------------------------------------------
Zcu111OranOsImage::Zcu111OranOsImage(ImageTask& task) :
	ImageBase(task, "Zcu111OranOsImage"),
...
//-------------------------------------------------------------------------------------------------------------
Zcu111OranOsImage::~Zcu111OranOsImage()
{
}

//-------------------------------------------------------------------------------------------------------------
unsigned Zcu111OranOsImage::numFiles() const
{
...
//-------------------------------------------------------------------------------------------------------------
unsigned Zcu111OranOsImage::installTimeSecs() const
{
	// calc the install time for the image based on size estimate (because the partition size is HUGE
	// and would lead to massive estimate!)
	float estimate(IMAGE_SIZE * TIMING_FACTOR) ;

	// add 30s margin
	estimate += 30 ;

	// add in BOOT.BIN if flashing it
	if (mUbootInstall)
	{
		estimate += (float)mDefaultFlashTimes.at(PARTITION_BOOT) ;
		estimate += 30 ;
	}

	return (unsigned)estimate ;
}

//-------------------------------------------------------------------------------------------------------------
bool Zcu111OranOsImage::initialise(std::string& failReason)
{
...
//-------------------------------------------------------------------------------------------------------------
std::string Zcu111OranOsImage::formatName(const std::string& version, const std::string& buildNumber)
{
...
//-------------------------------------------------------------------------------------------------------------
void Zcu111OranOsImage::show(std::ostream& os) const
{
...
//-------------------------------------------------------------------------------------------------------------
std::string Zcu111OranOsImage::info() const
{
	std::string info("") ;
...
//-------------------------------------------------------------------------------------------------------------
bool Zcu111OranOsImage::setUbootInstall(bool ubootInstall, std::string& failReason)
{
...
//-------------------------------------------------------------------------------------------------------------
IImageTypes::OsImageBank Zcu111OranOsImage::getOsBank(void)
{
	return mBootBank ;
}

//-------------------------------------------------------------------------------------------------------------
bool Zcu111OranOsImage::getUbootInstall(void)
{
	return mUbootInstall ;
}

//-------------------------------------------------------------------------------------------------------------
bool Zcu111OranOsImage::usePrevious(std::string failReason)
{
...
//-------------------------------------------------------------------------------------------------------------
bool Zcu111OranOsImage::clean(std::string &failReason)
{
	// Prevent normal operations from deleting any files that may be allocated to a slot
	// Leave all the cleaning to the AppImage
	return true ;
}
...
//-------------------------------------------------------------------------------------------------------------
bool Zcu111OranOsImage::alreadyDownloaded()
{
	...
	// fall back on checking the downloaded file
	return this->ImageBase::alreadyDownloaded() ;
}

//-------------------------------------------------------------------------------------------------------------
bool Zcu111OranOsImage::alreadyInstalled()
{
...
//-------------------------------------------------------------------------------------------------------------
bool Zcu111OranOsImage::download(std::string& failReason)
{
	...
	keepAlive() ;
	if (!downloadFunc(imageName, serverUrl, installDir, failReason))
		return false ;

	keepAlive() ;
	if (!downloadFunc(md5Name, serverUrl, installDir, failReason))
		return false ;
...
//-------------------------------------------------------------------------------------------------------------
bool Zcu111OranOsImage::check(std::string& failReason)
{
	std::string imageName(getImageName()) ;
	std::string md5Name(getMd5Name()) ;

	bool ok = checkFile(getInstallDir(), imageName, md5Name, failReason) ;
...
//-------------------------------------------------------------------------------------------------------------
bool Zcu111OranOsImage::install(std::string& failReason)
{
	...
	else
	{
		...
		Path::removeDirContents(osTempDir) ;
...
//-------------------------------------------------------------------------------------------------------------
bool Zcu111OranOsImage::flashNor(const std::string& filePath,
		const std::string& partition, std::string& failReason)
{
	...
	statusObj->end();
#else
	unsigned size(Path::fileSize(filePath)) ;
	unsigned expectedSecs( (unsigned)( (float)size * TIMING_FACTOR) ) ;

	bool timedOut(false) ;
    std::shared_ptr<SystemTask> systemTask( new SystemTask() ) ;
    systemTask->start();

	logDebugVerbose("CMD TIMEOUT: %u secs", expectedSecs) ;
	...
	int ret = systemTask->runCmd(cmd,
		...
		timedOut);
	statusObj->end();
	logDebugVerbose("CMD COMPLETE: status=%d timedout=%u", ret, (unsigned)timedOut) ;
	if (ret != 0)
	{
		if (timedOut)
		{
...
//-------------------------------------------------------------------------------------------------------------
bool Zcu111OranOsImage::flashBank(IImageTypes::OsImageBank bank, const std::string& filePath,
		const std::string& partition, std::string& failReason)
{
...
//-------------------------------------------------------------------------------------------------------------
bool Zcu111OranOsImage::manageLinks(IImageTypes::OsImageBank bank, std::string& failReason)
{
	std::string installDir(getInstallDir()) ;
	std::string md5Name(getMd5Name()) ;
...
//-------------------------------------------------------------------------------------------------------------
std::string Zcu111OranOsImage::bankName(const std::string& name, IImageTypes::OsImageBank bank) const
{
...
//-------------------------------------------------------------------------------------------------------------
bool Zcu111OranOsImage::isBankInstalled(IImageTypes::OsImageBank bank) const
{
	std::string installDir(getInstallDir()) ;
	std::string pimage = installDir + "/" + bankName(imageLink, bank) ;
...
//-------------------------------------------------------------------------------------------------------------
bool Zcu111OranOsImage::isBankVersionInstalled(IImageTypes::OsImageBank bank) const
{
	std::string installDir(getInstallDir()) ;
	std::string md5Name(getMd5Name()) ;
...
//-------------------------------------------------------------------------------------------------------------
bool Zcu111OranOsImage::installBank(IImageTypes::OsImageBank bank, std::string& failReason)
{
	keepAlive() ;
	...
	std::string tgzPath = installDir + "/" + imageName ;
	...
	// Program images
	if (!flashBank(bank, kernel, PARTITION_IMAGE, failReason))
		return false ;

	if (mUbootInstall)
		if (!flashNor(boot, PARTITION_BOOT, failReason))
			return false ;
...
//-------------------------------------------------------------------------------------------------------------
IImageTypes::OsImageBank Zcu111OranOsImage::getOtherBank(IImageTypes::OsImageBank bank) const
{
	if (bank == IImageTypes::OS_BANK1)
...
//-------------------------------------------------------------------------------------------------------------
IImageTypes::OsImageBank Zcu111OranOsImage::getActiveBank() const
{
...
//-------------------------------------------------------------------------------------------------------------
bool Zcu111OranOsImage::setActiveBank(IImageTypes::OsImageBank bank)
{
...
//-------------------------------------------------------------------------------------------------------------
void Zcu111OranOsImage::skipUboot()
{
	std::shared_ptr<IFileStatus> statusObj(getStatusObject()) ;
...
//-------------------------------------------------------------------------------------------------------------
void Zcu111OranOsImage::skipBank()
{
	std::shared_ptr<IFileStatus> statusObj(getStatusObject()) ;
...
//-------------------------------------------------------------------------------------------------------------
void Zcu111OranOsImage::skipFiles()
{
	// skip boot1 & boot2
	skipBank() ;
	skipBank() ;
