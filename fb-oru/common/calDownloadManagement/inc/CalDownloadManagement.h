...
#ifndef CALDOWNLOADMANAGEMENT_H_
#define CALDOWNLOADMANAGEMENT_H_
...
class CalDownloadManagement : public ICalDownloadManagement, public Task
{
public:
	virtual ~CalDownloadManagement() ;
	 ...
	 */
	virtual bool addCalDownloadType( std::shared_ptr<ICalDownload> calDownLoad ) override;
     ...
     * @return
     */
    virtual std::vector< std::shared_ptr<ICalDownload> > getCalDowloadObjects() override;
...
protected:
    ...
    // Get the cal download object (mainly for test purposes)
    std::shared_ptr<ICalDownload> getCalDownload(const std::string& typeName) const ;
...
private:
    ...
    std::shared_ptr<ICalDownload> mDownload ;
    bool mDownloadOk ;
    ...
    typedef std::map< std::string, std::shared_ptr<ICalDownload>> MapCalDownload;

    // Keep a map of the Image objects used to manage each image type
    MapCalDownload mCalDownloadMap ;

} ;

}

#endif /* CALDOWNLOADMANAGEMENT_H_ */
