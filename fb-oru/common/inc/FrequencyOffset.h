...
public:
    ...
    /**
     * FrequencyOffset object constructor that allows the initial frequency value to be
     ...
     * @param unit default to hertz but can be overridden to any FrequencyOffset::Unit type
     */
	FrequencyOffset(long long frequency, FrequencyOffset::Unit unit = FrequencyOffset::Hz);



	/**
	 * Conversion function to convert from base type to this type (just some glue to make the operators
	 * work as expected)
...
protected:
    /**
     * Hook called when value may be going negative. For types that don't support negative values, you can either
...
inline std::ostream& operator << (std::ostream& os, const Mplane::FrequencyOffset& frequency)
{
	os << frequency.getInt(Mplane::FrequencyOffset::KHz) << " " << frequency.unit2Str( Mplane::FrequencyOffset::KHz) ;
	return os;
}
