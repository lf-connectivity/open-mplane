 ...
 *
 * \file      Service.h
 * \brief     Defines the base class functionality and interface that must be supported by all services.
 *
 *
 * \details   Defines the base class functionality and interface that must be supported by all services.
 *
 */
...
/**
 * This is the base class that must be implemented by all radio services. It provides
 * a common mechanism through which the service can be controlled and execute.
 *
 ...
 * the service state. The initial service state is ServiceCreated, this indicates
 * that the Service object has been created and is waiting to be started. The service
 * design must be such that other threads can access the service data once it is created.
 * This must include the ability for service observers to register for event notification.
 *
 ...
 *
 * Some services are expected to be able to reset to a known initial state, these
 * services must overload the reset() method and return their data and state to
 * its start condition.
 * @note This MUST NOT include the modification of any registered data observers.
 *
...
public:
    ...
    /**
     * Method called by the controlling radio process to shutdown the service after
     * which the thread associated with service will be destroyed
     *
    ...
    /**
     * Helper method used to return the name associated with a service control type
...
protected:
     ...
     *
     * The default constructor will create a medium prority task , with a small stack
     * that calls its service() method once a second.
     *
     * @param priority a prority between SERVICE_MINIMUM_PRIORITY and SERVICE_MAXIMUM_PRIORITY
     * @param stacksize One of the available Task defined stack sizes of SMALL_STACK, MEDIUM_STACK or LARGE_STACK
     * @param msecTimerPeriod a millisecond update time from 10 to 86,400,000
     */
    Service( ServicePriority_t priority = SERVICE_MEDIUM_PRIORITY,
    ...
    /**
     * This method can be used in the implementation of the public Mplane::Observer
     * methods that provide data to a service in order for the service thread to be
     * Signalled and as a result the service can act upon the data change immediately
     * or at least when it is scheduled to run.
     */
    void eventNotify();
