...
/**
 * Expands any variables within the string, replacing the variable name with it's value. If the variable is not set then
 * the variable is replaced with an empty string.
 *
 * Variable is defined in teh format ${NAME} where 'NAME' is the variable name.
 *
 ...
 */
inline std::string expandStr(const std::string& str, const std::map<std::string, std::string>& vars)
{
	if (str.empty())
		return "" ;
 ...
 */
inline std::string macToStr(uint64_t macid)
{
	std::stringstream ss ;
 ...
 */
inline uint64_t strToMac(const std::string& _macstr)
{
 ...
 */
inline std::string charpToString(const char* cp)
{
	if (!cp)
		return "" ;
 ...
 */
inline std::string convertHexToString(std::string hexStr){
	std::string ascii = "";
