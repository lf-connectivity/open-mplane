...
/**
 * Wrapper class which creates an object that is an Observer of some Subject and which passes any update() on to
 * the parent class which created it
 */
template<typename Tparent, typename Tsubject>
class ObserverWrapper : public Observer<Tsubject>
{
public:
	ObserverWrapper(Tparent* parent, const std::string& name, std::shared_ptr<Tsubject> subject) :
		mParent(parent),
		mName(name),
		mSubject(subject)
	{
...
/**
 * Utility template function which creates a shared pointer to an ObserverWrapper object
 * @param parent
 * @param name
 * @param subject
