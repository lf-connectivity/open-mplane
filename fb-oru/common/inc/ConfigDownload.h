...
/**
 * This base class implementation is intended to be used by objects that read configuration data files
 * that can be maintained by the configuration data download manager.
 *
 * The final implementation register with the configuration data download manager so that it knows
 * with files types it should support. This makes the download manager functionality independent of
 * the configuration data implementation from one product to the next.
 */
...
public:
	...
	/**
	 * NOTE: Normally the IConfigDownload objects have name of the form "<name>ConfigDownload". Since we use this types list
	 * in tif as an enumerated list, we don't need the extra "ConfigDownload" string, so that gets silently removed.
	 */
	static std::string CONFIG_TYPE_SUFFIX ;

    /**
     * Get the name associated with this configuration data down load type. This name is used to derive the
     * hash value.
     */
    virtual std::string getName() const override;

	/**
	 * Get the type name associated with this configuration data down load type. If the name is of the form
	 * <Type>ConfigDownload, then the string returned by this method will be <Type>
	 */
    ...
    /**
     * Adds a path and it's configuration date - used to display the list of config dates for each path
     */
	 ...
	 * via TCP port
	 */
	virtual std::shared_ptr<IFileMgrWriter> getConfigFileWriter(std::shared_ptr<IFileStatus> statusObj, unsigned fileSize,
