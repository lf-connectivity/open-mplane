 ...
 * \brief     Task which operates as the result of an event OR after a fixed period
 *
 *
 * \details   Task which operates immediately on an asynchronous event. If no event has occurred after a fixed period
 * 			then task will operate anyway.
 *
 */
...
public:
	TaskPeriodicEvent(unsigned periodMs,
			int priority = 0, StackSize stacksize = SMALL_STACK,
...
protected:
    ...
    Conditional mEvent ;
    unsigned mPeriodMs ;
    bool mPeriodicEvent ;	//<! Set if event is due to periodic timeout rather than asynchronous event
} ;
