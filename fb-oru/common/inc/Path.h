...
/**
 * Class wrapper for std::ifstream. Uses RAII to auto-close when object goes out of scope
 */
class IfStream {
public:
	/**
...
public:
	...
	/**
	 * Delete all of the files in a directory. Optionally recursively delete any sub-directories
	 * @param path
	 * @return true if ok
	 */
	static bool removeDirContents(const std::string& path, bool recursive = false);
	...
	/**
	 * Read the link specified by path. If able to, sets 'link' to the path the link points to
	 * @param path
	...
	/**
	 * Create a link from 'link' to 'path'. 'path' is the full path to the link, 'link' is the contents of the
	 * link (i.e. what the link links to)
	 *
	 ...
	 */
	static std::vector<std::string> dirContents(const std::string& path, bool chronologial = false) ;
	...
	/**
	 * Converts path into an absolute path (if it isn't already). Uses rootdir as the base if path is relative.
	 * If rootdir is '.' or empty then uses the current working directory
	 */
	static std::string absolute(const std::string& path, const std::string& rootdir = ".") ;
	 ...
	 * @param src	File path
	 * @param dest	File path or directory path. If directory, then copies file in with same name
	...
	/**
	 * Gets the amount of free space left on the file system that contains this directory path (or file path)
	 * @param path
	...
	/**
	 * Compare text files, optionally skipping the first 'skipStart' lines before doing the comparison
	 * @param filename1
	 * @param filename2
	 * @param skipStart	Skip this number of lines before comparing. Useful for ignoring changed headers
	 * @return true if files are different; false if same
	 */
	static bool filesDifferent(const std::string& filename1, const std::string& filename2, unsigned skipStart=0);
