...
protected:
	...
	virtual std::string outputStart(const std::string& op, const std::string& filename) const ;
	virtual std::string outputPercent(unsigned percent) const ;
	virtual std::string outputEnd() const ;

private:
	// Updates the progress counts and returns true if the status has changed such that a new output is required
