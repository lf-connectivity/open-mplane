 ...
 */
CPPUNIT_TEST_SUITE_REGISTRATION( MutexTestCase );


WTask::WTask():Task(0, Task::SMALL_STACK),
        mCount(0), testHold(false)
{
    state = mt_idle;
}

void Mplane::WTask::setCount(int count)
{
    Mutex::Lock lock( mMutex );  // use the Mutex lock guard here
...
ResetTask::ResetTask(WTask &task):
...
void Mplane::MutexTestCase::unittest()
{
    cout << endl << "*** MutexTestCase unittest ***" <<  endl;

    wtask = new WTask();

    CPPUNIT_ASSERT(wtask != 0);

    inctask = new IncTask(*wtask);

    CPPUNIT_ASSERT(inctask != 0);

    resettask = new ResetTask(*wtask);
    ...
    CPPUNIT_ASSERT( wtask->mMutex.isLocked() ==  true );
    ...
    CPPUNIT_ASSERT( wtask->mMutex.isLocked() ==  false );
    CPPUNIT_ASSERT( wtask->mCount ==  1 );
