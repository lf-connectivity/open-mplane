 ...
 *
 *
 * \details   This file contains the implementation of the common task Task class.
 *
 */
#include <config.h>
#ifndef _REENTRANT
...
Task::Task(int priority, StackSize stacksize,const char* name, const char* suName) :
        Loggable( name, suName ),
        mRunForever( false ),
        mStackSize( stacksize ), mStack(0), mPriority(0),
        mTaskName( name ),
        mSubUnitIdName( suName ),
        mThread(0), mRunning(false), mCancelled(false), mRunExitValue(0),
        mParentPid(0), mThreadID(0),
        mKickCount(0),
        mTaskListRef( TaskList::getInstance() )
{
#ifdef DEBUG_TASK
std::cerr << "NEW Task " << name << ":" << suName << " @ " << this << std::endl ;
#endif
...
Task::~Task()
{
	...
	std::cerr << "Task::~Task " << getInstanceName() << ":" << getGroupName() << std::endl ;
#endif
...
int Task::start()
{
    ...
    if( !mRunning )
    {
        ...
        {
            std::cerr << "error in pthread_attr_init : " << strError(ret) << std::endl ;
            return ret;
        }

        if( (ret =pthread_attr_setinheritsched(&attr,PTHREAD_EXPLICIT_SCHED)) != 0)
        {
            std::cerr << "error in pthread_attr_setinheritsched : " << strError(ret) << std::endl ;
        }

//        if( (ret = pthread_attr_setschedpolicy(&mThreadAttribute, SCHED_FIFO)) != 0 )
        if( (ret = pthread_attr_setschedpolicy(&attr, SCHED_RR)) != 0 )
        {
            std::cerr << "error in pthread_attr_setschedpolicy : " << strError(ret) << std::endl ;
            return ret;
        }

        if( (ret = pthread_attr_setstacksize(&attr, mStack)) != 0)
        {
            std::cerr << "error in pthread_attr_setstacksize : " << strError(ret) << std::endl ;
            return ret;
        }
...
//        }
        ...
        if( (ret = pthread_attr_getschedparam(&attr, &param)) == 0)
        {
            ...
            if (ret)
            {
                std::cerr << "error in pthread_attr_setschedparam : " << strError(ret) << std::endl ;
                return ret;
            }
        }
        else
        {
            std::cerr << "error in pthread_attr_getschedparam : " << strError(ret) << std::endl ;
        }

        if( (ret = pthread_create(&mThread, attrp, &Task::startThread, this)) != 0)
        {
            std::cerr << "error in pthread_create : " << strError(ret) << " (" << ret << ")" << std::endl ;
#ifdef OFF_TARGET
            std::cerr << "You need to ensure /etc/security/limits.conf has been amended to include:" << std::endl ;
            std::cerr << "  @wtl_axis        -       memlock         51200" << std::endl ;
            std::cerr << "  @wtl_axis        -       rtprio          98" << std::endl ;
            std::cerr << "Please see \"Software Development Environment\" in Radio Software Evolution documents area." << std::endl ;
#endif
        }
        ...
        CPU_ZERO(&cpuset);
        CPU_SET(2, &cpuset);
        if( (ret = pthread_setaffinity_np(mThread, sizeof(cpu_set_t), &cpuset)) != 0)
        {
            std::cerr << "Failed to set CPU affinity for thread : " << strError(ret) << std::endl ;
        }
#endif
    }
    return ret;
...
void Task::execute()
{
	...
	std::cerr << "Task::execute " << getInstanceName() << ":" << getGroupName() << std::endl ;
#endif
...
void Task::exit()
{
	...
	std::cerr << "Task::exit " << getInstanceName() << ":" << getGroupName() << std::endl ;
#endif
...
const char* Task::getSubUnitId() const
{
...
void* Task::startThread(void* task)
{
#ifdef DEBUG_TASK
...
pthread_t Mplane::Task::getId()
{
    return mThread;
}

pid_t Mplane::Task::getParentPID()
{
    return mParentPid;
}

pid_t Mplane::Task::getThreadID()
{
    return mThreadID;
}

void Mplane::Task::kickSoftwareWatchdog()
{
    ++mKickCount;
}

int Mplane::Task::getKickCount()
{
    return mKickCount;
}

bool Mplane::Task::isRunForever()
{
    return mRunForever;
}

int Mplane::Task::stop()
{
#ifdef DEBUG_TASK
	std::cerr << "Task::stop @ " << this << std::endl ;
	std::cerr << "Task::stop " << getInstanceName() << ":" << getGroupName() << std::endl ;
#endif

    int result;
    void* res;

    // NOTE: stop() and exit() may occur at the same time, so cope with mThread going away
    if (mThread == 0)
    	return 0 ;
...
void Mplane::Task::cleanup()
{
    // virtual that does nothing here but may be implemented by task implementations.
}

void Mplane::Task::cancelThread(void* task)
{
#ifdef DEBUG_TASK
...
void Mplane::Task::cancel()
{
#ifdef DEBUG_TASK
	std::cerr << "Task::cancel @ " << this << std::endl ;
	std::cerr << "Task::cancel " << getInstanceName() << ":" << getGroupName() << std::endl ;
#endif

    //
    // Call the extending classes cleanup method so they can tidy things
    // up when the the underlying task )thread) they are running in is being cancelled.
    //
    cleanup();
