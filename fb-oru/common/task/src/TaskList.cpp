...
std::shared_ptr<TaskList> TaskList::getInstance()
{
...
void TaskList::remove(Task* task)
{
    ...
    for ( taskIterator = mTaskList.begin();
            taskIterator != mTaskList.end();
            ++taskIterator )
    {
...
void TaskList::show(std::ostream& os)
{
    ...
    os << padLeft("TDI", TID_WIDTH) << padLeft("SUID", SUID_WIDTH) << padLeft("Name", NAME_WIDTH) << padLeft("State", STATE_WIDTH) << padLeft("PRI", PRIORITY_WIDTH) << std::endl ;
    os << padLeft("---", TID_WIDTH) << padLeft("----", SUID_WIDTH) << padLeft("----", NAME_WIDTH) << padLeft("-----", STATE_WIDTH) << padLeft("---", PRIORITY_WIDTH) << std::endl ;
...
bool TaskList::allRunning(const char** failedName)
{
    Mutex::Lock lock( mTaskListMutex );
...
Task* TaskList::checkStuckTask()
{
            ...
            ++taskIterator )
    {
        ...
        if( task != 0 )
        {
            // only check the stuck status of tasks that have been marked as run forever.
            if( task->isRunForever() )
            {
                // check to see if the thread associated with the task is still active
