...
public:
	 ...
	 */
	bool setGroupLevel(ILoggable::LogDebugLevel level, const std::string& groupName) ;
	 ...
	 */
	bool setInstanceLevel(ILoggable::LogDebugLevel level, const std::string& instanceName) ;
	...
	/**
	 * Given a group name, ensures it is valid (i.e. set to something and has no spaces)
	 ...
	 * @return valid name
	 */
	std::string validName(const std::string& name, const std::string& defaultName) const ;

	/**
	 * Make settings persistent over reboots (uses u-boot environment)
	 * @return
	 */
	bool save() const ;

	/**
	 * Stop registering any new objects (i.e. once application has initialised). Any subsequent objects attempting to register
...
private:
	/**
	 * Get the current group level using a case-insensitive match on the name. If there is no current setting then
	 * create one using the default value specified
	 * @param groupName
	 * @param defaultLevel
	 * @return group level
	 */
	ILoggable::LogDebugLevel getGroupLevel(const std::string& groupName, ILoggable::LogDebugLevel defaultLevel = ILoggable::LOG_DEBUG_NONE) ;
	 ...
	 * @param level
	 */
	void setGroupLevel(const std::string& groupName, ILoggable::LogDebugLevel level) ;

	/**
	 * Get the current instance level using a case-insensitive match on the name. If there is no current setting then
	 * create one using the default value specified
	 * @param instanceName
	 * @param defaultLevel
	 * @return instance level
	 */
	ILoggable::LogDebugLevel getInstanceLevel(const std::string& instanceName, ILoggable::LogDebugLevel defaultLevel = ILoggable::LOG_DEBUG_NONE) ;
	 ...
	 * @param level
	 */
	void setInstanceLevel(const std::string& instanceName, ILoggable::LogDebugLevel level) ;
