...
Mplane::ServicesMonitor::ServicesMonitor():
        Task(SERVICE_MONITOR_PRIORITY, Task::MEDIUM_STACK, "Services Monitor","SY"),
...
void Mplane::ServicesMonitor::startMonitoring()
{
    if( isRunning() == false )
    {
...
void Mplane::ServicesMonitor::applicationFailed()
{
...
void Mplane::ServicesMonitor::applicationStable()
{
    Conditional::Lock lock(mNotify);  // acquire conditional mutex, BSTS
...
IServicesMonitor::ApplicationState ServicesMonitor::getApplicationState() const
{
	return mAppState ;
}

int Mplane::ServicesMonitor::run()
{
...
void Mplane::ServicesMonitor::monitorServices()
{
    ...
    if( ++mServiceCheckCount == SERVICE_CHECK_RATE )
    {
        ...
        sprintf(time_str,"%4d-%02d-%02d %02d:%02d:%02d:", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);

        logDebugNormal("%s Services monitor - check for stuck or cancelled tasks",time_str);
        mServiceCheckCount = 0;
        if( TaskList::getInstance()->allRunning( &failedService ) == false )
        {
            if( mServiceDied == false)
                logDebugNormal( "%s Critical error: %s has failed - restart", time_str, failedService );
            mServiceDied = true;
        }
        ...
        if( task != 0)
        {
            if( mServiceStuck == false )
                logDebugNormal("%s Critical error: task %s is a CPU hog - for now cancel thread",time_str, task->getName());
//            pthread_kill( task->getId(), SIGQUIT ); // for now kill the task causing the problem
...
void ServicesMonitor::updateStateFile()
{
