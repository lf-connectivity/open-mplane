...
//----------------------------------------------------------------------------------------------------------------------
/**
 * The Mplane::DataVariant class is used to carry data between the RTI and the underlying
 * application code that implements the actual functions that call the code base.
 *
 * The intermediate function, depending on its parameter requirements, is passed a list of
 * one or more DataVariant object. The values of the objects are set and passed into the function
 * and the function may also set the value to be return to the caller.
 *
 * For more details of how the DataVariant is used by the RTI please see the higher level
 * RTI documentation.
 */
...
public:
     ...
     */
    DataVariantBase(const DataVariantType::Type& type, const std::string& name, const std::string& value = "" );
     ...
     * prior to calling this method so that the valid enumeration values can be determined.
     *
    ...
    /**
     * Method used to return the data variant value string. If the variable is optional and an empty string
     ...
     * an equals sign.
     *
     * @param name Optional name that allows you to override the variable name for display purposes
     ...
     */
    virtual std::string getNameValuePairOptional(const std::string& name = "") const;
    ...
    /**
     * Resets the variable to the empty state (type set to None) and all values/flags cleared
     */
    virtual void reset() ;
