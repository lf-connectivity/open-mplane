...
//-------------------------------------------------------------------------------------------------------------
bool DataVariantComplex::setComplex(const std::complex<float>& value)
{
	...
	{
		std::shared_ptr<IDvComplex> dv(std::dynamic_pointer_cast<IDvComplex>(mDvRef)) ;
		return dv->setComplex(value) ;
	}

	/* Need to convert from specific type to intermediate value. use intermediate value to set */
	return mDvRef->fromInterFloat(static_cast<IDvAccess::InterFloat>(value.real())) ;
}

//-------------------------------------------------------------------------------------------------------------
std::complex<float> DataVariantComplex::toComplex() const
{
	/* If current type is already Complex, then use it directly */
	if (mDvRef->getType() == DataVariantType::Complex)
	{
		std::shared_ptr<IDvComplex> dvComplex(std::dynamic_pointer_cast<IDvComplex>(mDvRef)) ;
