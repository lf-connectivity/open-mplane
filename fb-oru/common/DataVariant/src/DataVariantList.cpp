...
//-------------------------------------------------------------------------------------------------------------
DataVariantList::DataVariantList(const char* fieldList) :
	errorVar("")
{
	addFields(fieldList) ;
}
...
//-------------------------------------------------------------------------------------------------------------
void DataVariantList::addFields(const char* fieldList)
{
    ...
    while (varTokens.nextToken() )
    {
        ...
        if ((limStart != std::string::npos) && (limEnd != std::string::npos) && (limEnd > limStart) )
        {
...
//-------------------------------------------------------------------------------------------------------------
bool DataVariantList::add(DataVariant* variant)
{
...
//-------------------------------------------------------------------------------------------------------------
DataVariant* DataVariantList::find(const std::string &name)
{
...
//-------------------------------------------------------------------------------------------------------------
bool DataVariantList::add(const std::string& name, const  DataVariantType::Type& type)
{
...
//-------------------------------------------------------------------------------------------------------------
bool DataVariantList::set(std::string valuePairs)
{
    ...
    while( t.nextToken() )
    {
		...
		if (token.find("=") == std::string::npos)
    	{
			...
			// this is a single value - Try setting the current argument variable using this token as the value
    		(*iter)->set( token );

    		// check setting
    		if (!(*iter)->isSet())
    		{
    			errorVar = "Invalid value '" + token + "' for '" + (*iter)->getName() + "'";
    			return false ;
    		}
		...
		if ( isVector != variant->isVector() )
		{
			errorVar = name ;
			if (isVector)
				errorVar = std::string("Specified vector assignment for non-vector ") + name ;
			else
				errorVar = std::string("Specified non-vector assignment for vector ") + name ;
		...
		// Set the iterator to this variable, then step 1 past it ready for next time round
		iter = findIter(name) ;
		if (iter != list.end())
			++iter ;
...
//-------------------------------------------------------------------------------------------------------------
std::string DataVariantList::toString(const std::string& separator)
{
    std::string result="";
...
//-------------------------------------------------------------------------------------------------------------
std::string DataVariantList::toStringFull(const std::string& separator)
{
    std::string result="";
...
//-------------------------------------------------------------------------------------------------------------
std::vector<std::string> DataVariantList::toStrings()
{
...
//-------------------------------------------------------------------------------------------------------------
std::vector<std::string> DataVariantList::toStringsFull()
{
	std::vector<std::string> result;
...
//-------------------------------------------------------------------------------------------------------------
std::vector<std::pair<std::string, std::string> > DataVariantList::toVarVals()
{
    ...
    for (iter = list.begin(); iter != list.end(); ++iter)
    {
        ...
        if (varPtr->isVector())
        {
        	std::vector< std::pair<std::string, DataVariant> > vectorVars(varPtr->getList()) ;
        	std::vector< std::pair<std::string, DataVariant> >::iterator vecIter;
            for (vecIter = vectorVars.begin(); vecIter != vectorVars.end(); ++vecIter)
            {
...
//-------------------------------------------------------------------------------------------------------------
std::string DataVariantList::getError()
{
    return errorVar;
}

//-------------------------------------------------------------------------------------------------------------
std::string DataVariantList::varlist(const std::string& separator, bool showValue)
{
    std::string result="";
...
//-------------------------------------------------------------------------------------------------------------
bool DataVariantList::empty() const
{
    return list.empty();
}

//-------------------------------------------------------------------------------------------------------------
bool DataVariantList::superset(const std::string& fieldList) const
{
...
//-------------------------------------------------------------------------------------------------------------
bool DataVariantList::superset(const DataVariantList& other) const
{
    ...
    for (iter = other.list.begin(); iter != other.list.end(); ++iter)
    {
        ...
        std::vector< DataVariant*>::const_iterator thisIter( findIter(varPtr->getName()) ) ;
...
//-------------------------------------------------------------------------------------------------------------
bool DataVariantList::subset(const std::string& fieldList) const
{
	DataVariantList other(fieldList.c_str()) ;
...
//-------------------------------------------------------------------------------------------------------------
bool DataVariantList::subset(const DataVariantList& other) const
{
    ...
    for (iter = list.begin(); iter != list.end(); ++iter)
    {
        ...
        std::vector< DataVariant*>::const_iterator otherIter( other.findIter(varPtr->getName()) ) ;
...
//-------------------------------------------------------------------------------------------------------------
void DataVariantList::clearValues()
{
    std::vector< DataVariant*>::iterator iter;
    for (iter = list.begin(); iter != list.end(); ++iter)
    {
...
//-------------------------------------------------------------------------------------------------------------
bool DataVariantList::checkLimits(std::string& error)
{
	bool ok(true) ;
	error = "" ;
	for (auto var : list)
...
//-------------------------------------------------------------------------------------------------------------
bool DataVariantList::checkLimits()
{
...
//-------------------------------------------------------------------------------------------------------------
std::vector<DataVariant> Mplane::DataVariantList::getList() const
{
...
//-------------------------------------------------------------------------------------------------------------
std::vector< DataVariant*>::iterator DataVariantList::findIter(const std::string &name)
{
...
//-------------------------------------------------------------------------------------------------------------
std::vector< DataVariant*>::const_iterator DataVariantList::findIter(const std::string &name) const
{
	std::string lowerName(toLower(name)) ;
    std::vector< DataVariant*>::const_iterator iter;
