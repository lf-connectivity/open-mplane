...
bool Tokeniser::nextToken()
{
    return nextToken(mDelimiters);
}

bool Tokeniser::nextToken(const std::string& delimiters)
{
...
bool Tokeniser::hasNextToken(const std::string& delimiters) const
{
    size_t index = mString.find_first_not_of(delimiters, mOffset);
    if (std::string::npos == index)
    {
...
const std::string Tokeniser::getToken() const
{
    return mToken;
}
...
char Tokeniser::getDelimiter() const
{
    return mDelimiter;
}
