...
namespace {
	...
	const float getUnitFactor(Power::Unit unit)
	{
...
const char * Mplane::Power::unit2Str(Power::Unit unit)
{
    switch (unit)
    {
    case dBm:
...
Power::Unit Power::str2unit(const std::string& units)
{
...
void Mplane::Power::set(float power, Power::Unit unit)
{
...
void Mplane::Power::set(float power)
{
	// dBm
	mPower_dBm = power ;
    mWattsSet = false ;
...
void Mplane::Power::set(const std::string& power, Power::Unit unit)
{
...
void Mplane::Power::setWatts(float power)
{
    mPower_watts = power;
    mPower_dBm = convertWattsTodBm(mPower_watts);
    mWattsSet = true ;
...
float Mplane::Power::get(Power::Unit unit) const
{
	if (unitsInWatts(unit))
	{
		if (!mWattsSet)
...
float Mplane::Power::getWatts() const
{
	if (!mWattsSet)
		setPowerWatts() ;
...
int Mplane::Power::getInt(Power::Unit unit) const
{
	double dscaledPower = (double)get(unit) ;
	dscaledPower =  (dscaledPower - floor(dscaledPower) < 0.5 ? floor(dscaledPower) : ceil(dscaledPower));
...
Power& Mplane::Power::operator=(const Power& rhs)
{
...
Power Mplane::Power::operator=(const float power)
{
    mPower_dBm = power;
	setPowerWatts() ;
    return *this;
...
Power Mplane::Power::operator=(const int power)
{
...
Power Mplane::Power::operator+(const Power& rhs) const
{
    return Power(mPower_dBm + rhs.mPower_dBm);
}

Power Mplane::Power::operator+(const float power) const
{
    return Power(mPower_dBm + power);
}

Power Mplane::Power::operator+(const int power) const
{
...
float Mplane::Power::convertdBmToWatts(float powerdBm) const
{
    return (float)(exp10(powerdBm/10) / 1000);
}

float Mplane::Power::convertWattsTodBm(float powerWatts) const
{
    return (float)(10 * log10(1000 * powerWatts));
}

bool Mplane::Power::operator==(const Power& rhs) const
{
...
void Mplane::Power::setPowerWatts() const
{
