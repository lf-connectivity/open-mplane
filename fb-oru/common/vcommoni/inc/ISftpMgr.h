...
public:
	 ...
	 * @param auth  - indication of what the pass string contains i.e. how to authenticate
	 * @param error - set to an error string if unable to create session
	 */
	static std::shared_ptr<ISftpSession> factory(const std::string& host, const std::string& user,
	...
	/**
	 * Factory for creating SFTP sessions. May return null pointer
	 * @param remotePath	- Remote file path of the form: sftp://<username>@<host>[:port]/path
	 * @param authData	- string to contain password/key/certificate etc
	 * @param auth  - indication of what the pass string contains i.e. how to authenticate
	 * @param error - set to an error string if unable to create session
	 */
	static std::shared_ptr<ISftpSession> factory(const std::string& remotePath,
			const std::string& authData, ISftpMgr::AuthenticationType auth,
			const std::vector<ISshSession::PublicKey>& serverKeys,
			std::string& error) ;
