...
/**
 * File save virtual class. This class is used to manage data transferred primarily over TCP PORT IN ports, where the data is to be written
 * to a file. Each block of data added via this class will be appended to the end of the file.
 */
...
public:
	 ...
	 * @return true if able to switch to this directory (i.e. it exists etc)
	 */
	virtual bool setTempDir(const std::string& dir, bool mkdir = true, bool clear = true) =0 ;
	 ...
	 */
	virtual bool download(const std::string& url, const std::string& filename) =0 ;
	 ...
	 */
	virtual std::shared_ptr<IFileMgrWriter> portTransferStart(const std::string& filename, unsigned fileSize) =0 ;
	...
	virtual std::shared_ptr<IFileMgrWriter> portTransferStart(const std::string& filename,
			unsigned fileSize, std::shared_ptr<IFileStatus> status) =0 ;
