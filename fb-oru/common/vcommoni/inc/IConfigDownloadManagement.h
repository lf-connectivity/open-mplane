 ...
 *
 * The ConfigDownloadManagement class provides a singleton interface to the radio application's
 * config file download management task that is responsible for ensuring the application manages its
 * config file downloads within the application.
 *
 */
...
public:
    ...
    /**
     * Method used to add different configuration download types to the configuration data download
     * manager.
     *
     * @param ConfigDownload
     * @return true is added OK, otherwise false.
     */
    virtual bool addConfigDownloadType( std::shared_ptr<IConfigDownload> ConfigDownload ) = 0;


    /**
     * Return a list of configuration down load types registered with the configuration download manager.
     * @return
     */
    virtual std::vector< std::shared_ptr<IConfigDownload> > getConfigDowloadObjects() = 0;
