 ...
 *
 * \file      IConfigDownload.h
 * \brief     Interface class for individual config file download
 *
 *
 * \details   Interface to class that provides the methods to download config files
 *
 */


#ifndef ICONFIGDOWNLOAD_H_
#define ICONFIGDOWNLOAD_H_
...
class IConfigDownload
{
public:
	IConfigDownload() {}
	virtual ~IConfigDownload() {}
    ...
    /**
     * Adds a path and it's configuration date - used to display the list of cal dates for each path
     */
    virtual void addPath(unsigned path, const std::string& calDate) =0 ;


	/**
	 * Get the FileMgrWriter instance for installing config files. Used in preparation for installing a cal file
	 * via TCP port
	 */
