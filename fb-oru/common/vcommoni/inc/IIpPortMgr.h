 ...
 *
 *
 * \details   This singleton maintains a list of IP/port pairs which the application can then use
 *
 */
 ...
 * \class  IIpPortMgr
 * \brief  IP / port manager
 * \details   Writeable access to IP/port list
 *
 */
...
public:
	 ...
	 * NOTE: This does not apply the new list to the non-volatile store, it just updates the current list
	 */
	virtual void setList(const std::vector<std::pair<std::string, unsigned>>& list) =0 ;
