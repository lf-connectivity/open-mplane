...
public:
     ...
     * data files used by the various configuration data interfaces and populate the
    ...
    /**
     * Method used to indicate that the application has loaded its configuration data
     * @return
     */
 ...
 * all configuration data interfaces. This data must include a general information string and
 * version number.
 *
 * The load() method is called during initialisation to load the configuration data from
...
/**
 * The IConfigData interface provides a common base implementation that can be incrementally
 * applied to functionality associated with any number of radio types. It defines access
 * methods used to return the base configuration data interface common to a number of
 * types. This interface can be used to first load the configuration data and secondly
 * provide an interface to the basic configuration object that can be dynamically cast
 * to a known high level implementation.
 *
 * If a particular hardware configuration doesn't support a specific configuration data
...
public:
     ...
     *  configuration data object. This must be implemented using the singleton pattern by the
