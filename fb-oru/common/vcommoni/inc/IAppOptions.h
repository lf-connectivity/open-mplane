 ...
 * that can be defined when creating a new application that uses the Mplane::IAppOptions
 * class.
 */
...
public:
	 ...
	 */
	static std::shared_ptr<IAppOptionSpec> factory(const std::string& name, const DataVariantType::Type& type,
	 ...
	 * @param description	Description of option
	 * @param defaultValue	Default value to use if option not specified
	 * @param useEnvVar	If option is not specified on the command line, try using this environment variable. If that isn't specified
	 * 					either then use default value
	 * @return option specification
	 */
	static std::shared_ptr<IAppOptionSpec> factory(const std::string& name, const DataVariantType::Type& type,
...
/**
 * The IAppOptions class defines a pure virtual interface that must be implemented
 * by all Mplane software platform applications that implement the Mplane::IApplication
 * interface.
 *
 * It allows the application to be created with a number of startup options defined.
 */
class IAppOptions {
public:
	virtual ~IAppOptions() {}
