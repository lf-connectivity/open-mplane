...
/**
 * The ConfigCoordinator interface manages the configuration objects registered with it
 *
 */
...
public:
     ...
     */
    virtual ReturnType::State registerConfig(std::shared_ptr<IConfigDownload> cfgDownload,
...
private:
    ...
    std::map<std::string, std::tuple<std::shared_ptr<IConfigDownload>, std::shared_ptr<IConfigLoad>>> mConfigs;
