...
ConfigDownloadManagement::ConfigDownloadManagement() :
	Task(MANAGER_PRIORITY, Task::MEDIUM_STACK, "Config Download Mgr", "CONFIG"),
...
//-------------------------------------------------------------------------------------------------------------
void ConfigDownloadManagement::shutdown(void)
{
	mTaskRunning = false ;
...
//-------------------------------------------------------------------------------------------------------------
bool ConfigDownloadManagement::getAndInstall(const std::string& typeName,
...
//-------------------------------------------------------------------------------------------------------------
bool ConfigDownloadManagement::isDownloading(void) const
{
	Mutex::Lock lock(mMutex) ;
...
//-------------------------------------------------------------------------------------------------------------
bool ConfigDownloadManagement::isDownloadOk(std::string& failReason) const
{
	Mutex::Lock lock(mMutex) ;
	if (!mDownloadOk)
...
//-------------------------------------------------------------------------------------------------------------
bool ConfigDownloadManagement::setRemoteUrl(const std::string& url, std::string& failReason)
{
	Mutex::Lock lock(mMutex) ;
	if (mIsDownloading)
	{
...
//-------------------------------------------------------------------------------------------------------------
std::string ConfigDownloadManagement::getRemoteUrl()
{
	return mServerUrl ;
}

//-------------------------------------------------------------------------------------------------------------
void ConfigDownloadManagement::showDetail(std::ostream& os) const
{
}

//-------------------------------------------------------------------------------------------------------------
void ConfigDownloadManagement::show(std::ostream& os) const
{
...
//-------------------------------------------------------------------------------------------------------------
void ConfigDownloadManagement::show(const std::string& typeName, std::ostream& os) const
{
	auto configDownload(getConfigDownload(typeName)) ;
	if (!configDownload)
		return ;
...
//-------------------------------------------------------------------------------------------------------------
std::string ConfigDownloadManagement::getInstallFileName(const std::string& typeName) const
{
	auto configDownload(getConfigDownload(typeName)) ;
	if (!configDownload)
		return "";
...
//-------------------------------------------------------------------------------------------------------------
std::string ConfigDownloadManagement::getInstallFilePath(const std::string& typeName) const
{
	auto configDownload(getConfigDownload(typeName)) ;
	if (!configDownload)
		return "";
...
//-------------------------------------------------------------------------------------------------------------
bool ConfigDownloadManagement::testInjectInstallDir(const std::string& path, std::string& failReason)
{
	Mutex::Lock lock(mMutex) ;
...
//-------------------------------------------------------------------------------------------------------------
bool ConfigDownloadManagement::addConfigDownloadType( std::shared_ptr<IConfigDownload> configDownLoad )
{
	Mutex::Lock lock(mMutex) ;
...
//-------------------------------------------------------------------------------------------------------------
std::vector< std::shared_ptr<IConfigDownload> > ConfigDownloadManagement::getConfigDowloadObjects()
{
...
//-------------------------------------------------------------------------------------------------------------
std::set<std::string> ConfigDownloadManagement::getTypes() const
{
...
//-------------------------------------------------------------------------------------------------------------
std::shared_ptr<IFileMgrWriter> ConfigDownloadManagement::getConfigFileWriter(
		...
		unsigned fileSize, std::string& failReason)
{
	...
	std::shared_ptr<ConfigDownload> configDownload(std::dynamic_pointer_cast<ConfigDownload>(iconfigDownload)) ;
	return configDownload->getConfigFileWriter(statusObj, fileSize, failReason) ;
}

//-------------------------------------------------------------------------------------------------------------
bool ConfigDownloadManagement::configInstall(const std::string& typeName,
		std::shared_ptr<IFileStatus> status, ImageConfigWrite configWriteFunc,
		ImageConfigComplete configCompleteFunc, std::string& failReason)
{
	Mutex::Lock lock(mMutex) ;
...
//-------------------------------------------------------------------------------------------------------------
int ConfigDownloadManagement::run()
{
	/*
	...
	while (mTaskRunning)
	{
		...
		{
            logDebugNormal("ConfigDownloadManagement - start download : Type=%s", mTypeDownloading.c_str()) ;
			...
			logDebugNormal("ConfigDownloadManagement - download complete ok=%d (%s)", mDownloadOk, mDownloadFailReason.c_str() ) ;
		}
	}
...
//-------------------------------------------------------------------------------------------------------------
bool ConfigDownloadManagement::getState(std::string& typeName) const
{
	Mutex::Lock lock(mMutex) ;
...
//-------------------------------------------------------------------------------------------------------------
void ConfigDownloadManagement::setState(bool downloading, const std::string& typeName)
{
	Mutex::Lock lock(mMutex) ;
	mTypeDownloading = typeName ;
...
//-------------------------------------------------------------------------------------------------------------
std::shared_ptr<IConfigDownload> ConfigDownloadManagement::getConfigDownload(const std::string& typeName) const
{
	// IConfigDownload name suffix - can be stripped off
	static const std::string CONFIG_SUFFIX_UC{toUpper(ConfigDownload::CONFIG_TYPE_SUFFIX)} ;
...
//-------------------------------------------------------------------------------------------------------------
bool ConfigDownloadManagement::setRemoteUrlNoMutex(const std::string& url, std::string& failReason)
{
	mServerUrl = url ;
	for (auto& cs : mConfigDownloadMap)
	{
...
//-------------------------------------------------------------------------------------------------------------
void Mplane::ConfigDownloadManagement::setStatusObject(std::shared_ptr<IFileStatus> status)
{
    mStatusObject = status ;
	for (auto& cs : mConfigDownloadMap)
	{
