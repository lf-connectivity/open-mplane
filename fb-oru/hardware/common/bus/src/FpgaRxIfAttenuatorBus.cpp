 ...
 *
 * \file      FpgaRxIfAttenuatorBus.cpp
...
//-------------------------------------------------------------------------------------------------------------
FpgaRxIfAttenuatorBus::FpgaRxIfAttenuatorBus(const std::string& busName, const std::string& busDetails, std::shared_ptr<IBusLock> busLock,
...
//-------------------------------------------------------------------------------------------------------------
FpgaRxIfAttenuatorBus::~FpgaRxIfAttenuatorBus()
{
}
...
//-------------------------------------------------------------------------------------------------------------
bool FpgaRxIfAttenuatorBus::doWriteByte(uint32_t offset, uint8_t byte)
{
	/// Set the attenuation value
	mFpga->write_spi_rx_if_atten(mIndex, byte);

	/// Toggle the attenuation update bit
	mFpga->write_spi_rx_if_atten_update(0);
	usleep(50);
	mFpga->write_spi_rx_if_atten_update(1);
	...
	do
	{
		...
		if (mFpga->read_spi_rx_if_atten_done())
		{
			return true;
		}
