...
#ifndef FpgaCommsMSGLOG_H_
#define FpgaCommsMSGLOG_H_

#include "FpgaCommsMsgBase.h"

namespace Mplane {

class FpgaCommsMsgLog : public FpgaCommsMsgBase
{
public:
	/**
	 ...
	 * @param pdu
	 */
	FpgaCommsMsgLog(std::shared_ptr<IFpgaMsgTokens> tokens, std::shared_ptr<IFpgaPdu> pdu) ;
	 ...
	 * @param pdu
	 */
	FpgaCommsMsgLog(std::shared_ptr<IFpgaMsgTokens> tokens, const IFpgaPdu& pdu) ;

	/**
	 * Destructor
	 */
	virtual ~FpgaCommsMsgLog() ;
...
private:
	 ...
	 *
	 */
	FpgaCommsMsgLog(std::shared_ptr<IFpgaMsgTokens> tokens,
