 ...
 *
 * \file      RemoteCommsMsgBase.h
 * \brief     RemoteComms message base class
 *
 *
 * \details    RemoteComms messaging class that can take a command string and convert it into a PDU (with converted tokens).
 *
...
#ifndef RemoteCommsMSGBASE_H_
#define RemoteCommsMSGBASE_H_
...
class RemoteCommsMsgBase : public IFpgaMsg
{
public:
	 ...
	 * @param pdu
	 */
	RemoteCommsMsgBase(std::shared_ptr<IFpgaMsgTokens> tokens, std::shared_ptr<IFpgaPdu> pdu) ;
	 ...
	 * @param pdu
	 */
	RemoteCommsMsgBase(std::shared_ptr<IFpgaMsgTokens> tokens, const IFpgaPdu& pdu) ;
	 ...
	 *
	 */
	RemoteCommsMsgBase(std::shared_ptr<IFpgaMsgTokens> tokens,
...
protected:
	...
	bool readString(const std::string& data, unsigned & dataPos, std::string& value) const ;
...
private:
	...
	void addVariableString(std::vector<UINT8>& vec, unsigned short id, const std::string& value) const ;
