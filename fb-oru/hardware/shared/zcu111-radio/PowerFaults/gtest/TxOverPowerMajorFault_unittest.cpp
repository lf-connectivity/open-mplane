...
//-------------------------------------------------------------------------------------------------------------
TEST(TxOverPowerMajorFault, Test)
{
	...
	std::shared_ptr<IFault> faultTx1(make_fault<Tx1OverPowerFaultMajor>());
	std::shared_ptr<IFault> faultTx2(make_fault<Tx2OverPowerFaultMajor>());
	std::shared_ptr<IFault> faultTx3(make_fault<Tx3OverPowerFaultMajor>());
	std::shared_ptr<IFault> faultTx4(make_fault<Tx4OverPowerFaultMajor>());

	std::shared_ptr<IFaultOwner> faultOwner (make_faultOwner<TxOverPowerMajorFaultOwner>());
    ...
    fpgaEmu->reset() ;
    // start with no alarm
    fpgaEmu->writeField("shutdownAlarm[0]", 0) ;
    fpgaEmu->writeField("shutdownAlarm[1]", 0) ;
    ...
    // Alarm Tx1
    fpgaEmu->writeField("shutdownAlarm[0]", 1) ;
	EXPECT_FALSE(faultTx1->isFaultPresent()) ;
	faultOwner->updateFaults() ;
	EXPECT_FALSE(faultTx1->isFaultPresent()) ;
	faultOwner->updateFaults() ;;
    ...
    // Alarm Tx2
    fpgaEmu->writeField("shutdownAlarm[1]", 1) ;
	EXPECT_FALSE(faultTx2->isFaultPresent()) ;
	faultOwner->updateFaults() ;
	EXPECT_FALSE(faultTx2->isFaultPresent()) ;
	faultOwner->updateFaults() ;;
