 ...
 *
 * \file      TxOverPowerMajorFault_unittest.cpp
 * \brief     Google Test of SynthClockFault
 *
 *
 * \details   This file contains the implementation of the Google Test test cases
 *            for the TxOverPowerMajorFault class.
 *
 */
...
//-------------------------------------------------------------------------------------------------------------
TEST(TxOverPowerMinorFault, Test)
{
	...
	std::shared_ptr<IFault> faultTx1(make_fault<Tx1OverPowerFaultMinor>());
	std::shared_ptr<IFault> faultTx2(make_fault<Tx2OverPowerFaultMinor>());
	std::shared_ptr<IFault> faultTx3(make_fault<Tx3OverPowerFaultMinor>());
	std::shared_ptr<IFault> faultTx4(make_fault<Tx4OverPowerFaultMinor>());

	std::shared_ptr<IFaultOwner> faultOwner (make_faultOwner<TxOverPowerMinorFaultOwner>());
    ...
    fpgaEmu->reset() ;
    // start with no alarm
    fpgaEmu->writeField("highPowerAlarm[0]", 0) ;
    fpgaEmu->writeField("highPowerAlarm[1]", 0) ;
    ...
    // Alarm Tx1
    fpgaEmu->writeField("highPowerAlarm[0]", 1) ;
	EXPECT_FALSE(faultTx1->isFaultPresent()) ;
	faultOwner->updateFaults() ;
	EXPECT_FALSE(faultTx1->isFaultPresent()) ;
	faultOwner->updateFaults() ;;
    ...
    // Alarm Tx2
    fpgaEmu->writeField("highPowerAlarm[1]", 1) ;
	EXPECT_FALSE(faultTx2->isFaultPresent()) ;
	faultOwner->updateFaults() ;
	EXPECT_FALSE(faultTx2->isFaultPresent()) ;
	faultOwner->updateFaults() ;;
