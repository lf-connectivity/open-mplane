 ...
 *
 * \file      WaveformHardwareDpdAdc.h
 * \brief     Interfaces with the hardware to set up the waveform block specifically for DPD ADC
 *
 *
...
#ifndef WAVEFORMHARDWAREDPDADC_H_
#define WAVEFORMHARDWAREDPDADC_H_
 ...
 */
class WaveformHardwareDpdAdc : public WaveformHardware {
public:
	using super = WaveformHardware ;

	explicit WaveformHardwareDpdAdc(unsigned index, unsigned port) ;
	virtual ~WaveformHardwareDpdAdc() ;
...
inline std::ostream& operator<<(std::ostream& os, const Mplane::WaveformHardwareDpdAdc& rhs)
{
	rhs.show(os) ;
	return os ;
}

inline std::ostream& operator<<(std::ostream& os, const std::shared_ptr<Mplane::WaveformHardwareDpdAdc>& rhs)
{
	rhs->show(os) ;
	return os ;
}
...
#endif /* WAVEFORMHARDWAREDPDADC_H_ */
