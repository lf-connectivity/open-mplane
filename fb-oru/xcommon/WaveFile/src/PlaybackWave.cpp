...
//-------------------------------------------------------------------------------------------------------------
// RAII for encapsulated/encrypted file. Automatically closes file when object is destroyed
class EncapFile {
public:
	EncapFile() :
		mEncapsulatedFilepath(""),
		mInFile(),
		mFileSize(0),
		mBytesLeft(0)
	{}
	...
	bool readFile(std::vector<uint8_t>& data, unsigned numBytes)
	{
		if (mEncapsulatedFilepath.empty())
			return false ;
...
//-------------------------------------------------------------------------------------------------------------
// Playback file name information. Test model files have a fixed format filename. This class uses that formatting
...
public:
	...
	const std::string& getAirStd() const {
		return mAirStd;
	}
...
private:
	static void removePath(const std::string& filepath, std::string& path, std::string& sep, std::string& file)
	{
	...
	static void removeExtension(const std::string& name, std::string& base, std::string& ext)
	{
...
//-------------------------------------------------------------------------------------------------------------
bool PlaybackWave::encapsulate(const std::string& inFilename,
...
//std::cerr << "ecapsulate(" << inFilename << ") size=" << bytesLeft << std::endl ;
...
//unsigned block{0} ;
	...
	{
//		std::cerr << "ecapsulate block " << block << ": bytes left=" << bytesLeft << std::endl ;
		if (!encapsulateBlock(dataStart, dataStart+BLOCK_SIZE, outData))
			return false ;
		dataStart += BLOCK_SIZE ;
		bytesLeft -= BLOCK_SIZE ;
//		std::cerr << "ecapsulated block " << block << " : bytes left=" << bytesLeft << std::endl ;
//++block ;
	}

	// pack up what's left
//	std::cerr << "ecapsulated blocks : bytes left=" << bytesLeft << std::endl ;
	if (bytesLeft > 0)
	{
//		std::cerr << "ecapsulate final block : bytes left=" << bytesLeft << std::endl ;
...
//-------------------------------------------------------------------------------------------------------------
bool PlaybackWave::encapsulate(const std::string& inFilename,
		const std::vector<uint8_t>& inData, const std::string& outFilename)
{
...
//-------------------------------------------------------------------------------------------------------------
std::string PlaybackWave::getError()
{
	std::string error("") ;
...
//-------------------------------------------------------------------------------------------------------------
void Mplane::PlaybackWave::setFilepath(const std::string& filepath)
{
	mFileInfo = std::make_shared<PlaybackFileInfo>(filepath) ;
}

//-------------------------------------------------------------------------------------------------------------
bool Mplane::PlaybackWave::loadEncapsulated()
{
#ifdef USE_UNECRYPTED_PLAYBACK
	// Shouldn't ever be called when compiling to use unencrypted file
	setError("Cannot open encrypted file '" + mFileInfo->getFilepath() + "' when compiled to use unencrypted files") ;
	return false ;
#endif
...
//-------------------------------------------------------------------------------------------------------------
bool Mplane::PlaybackWave::loadEncapsulated(const std::string& filepath)
{
...
//-------------------------------------------------------------------------------------------------------------
bool Mplane::PlaybackWave::loadNextBlock(std::vector<uint8_t>& data, bool& eof)
{
#ifdef USE_UNECRYPTED_PLAYBACK
	// Shouldn't ever be called when compiling to use unencrypted file
	setError("Cannot load encrypted block when compiled to use unencrypted files") ;
	return false ;
#endif
...
//	std::cerr << "Block type 0x" << std::hex << (unsigned)type << std::dec << std::endl ;
//	std::cerr << "Wave data ( " << std::distance(dataStart, dataEnd) << "bytes):" << std::endl ;
//	ByteTransfer::dump(dataStart, dataEnd) ;


	// process data
	std::vector<uint8_t> decrypted ;
	if ( (type & IWaveFile::WaveBlockType::ENCRYPTED) == IWaveFile::WaveBlockType::ENCRYPTED)
	{
...
//	std::cerr << "After decryption ( " << std::distance(dataStart, dataEnd) << "bytes):" << std::endl ;
//	ByteTransfer::dump(dataStart, dataEnd) ;

	std::vector<uint8_t> decompressed ;
	if ( (type & IWaveFile::WaveBlockType::COMPRESSED) == IWaveFile::WaveBlockType::COMPRESSED)
	{
//		std::cerr << "Decompress ( " << std::distance(dataStart, dataEnd) << "bytes):" << std::endl ;
...
//		std::cerr << "After decompression ( " << std::distance(dataStart, dataEnd) << "bytes):" << std::endl ;
//		ByteTransfer::dump(dataStart, dataEnd) ;
	}
...
//-------------------------------------------------------------------------------------------------------------
bool PlaybackWave::checkFeatures(std::shared_ptr<IFeature> features)
{
	...
	// we only need to feature validate licensed files supplied with the E6610A, they all
    // have the ewf, the encrypted waveform file extension
	if( mFileInfo->getExtension() == ".ewf" )
	{

	    // create the feature string of the form <AIRSTD>_<DUPLEX>_<TM> and see if it's enabled
        ...
        {
            setError("Feature " + featureString + " not licensed for use in this application") ;
            return false ;
        }
	}
	return true ;
...
//-------------------------------------------------------------------------------------------------------------
unsigned PlaybackWave::dataSize()
{
	unsigned numBytes{0} ;
...
std::cerr << "PlaybackWave::dataSize() " << getFilepath() << " " << numBytes << std::endl ;
	return numBytes ;
}
...
//-------------------------------------------------------------------------------------------------------------
void PlaybackWave::setError(const std::string& error)
{
	if (!mError.empty())
		mError += "\n" ;
	mError += error ;
...
//-------------------------------------------------------------------------------------------------------------
bool Mplane::PlaybackWave::encapsulateBlock(
...
//	std::cerr << "ecapsulateBlock() block size=" << std::distance(dataStart, dataEnd) << std::endl ;
//	std::cerr << "Output Data so far:" << std::endl ;
//	ByteTransfer::dump(outData) ;

//	std::cerr << "Input ( " << std::distance(dataStart, dataEnd) << "bytes):" << std::endl ;
//	ByteTransfer::dump(dataStart, dataEnd) ;
	...
	{
//		std::cerr << " * compressed size=" << compressedData.size() << std::endl ;
...
//	std::cerr << "Encrypt input ( " << std::distance(dataStart, dataEnd) << "bytes):" << std::endl ;
//	ByteTransfer::dump(dataStart, dataEnd) ;
...
//	std::cerr << "Encrypted ( " << encryptedData.size() << "bytes):" << std::endl ;
//	ByteTransfer::dump(encryptedData) ;
