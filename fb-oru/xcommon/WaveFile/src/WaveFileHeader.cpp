...
static const unsigned HEAD_BYTES{MAGIC_SIZE+DATA_TYPE_SIZE+HEAD_VER_SIZE+HEAD_LEN_SIZE} ;

// Data block header
static const unsigned BLOCK_TYPE_SIZE{1} ;
static const unsigned BLOCK_UNUSED_SIZE{3} ;
static const unsigned BLOCK_SIZE{4} ;
static const unsigned BLOCK_HEAD_BYTES{BLOCK_TYPE_SIZE+BLOCK_UNUSED_SIZE+BLOCK_SIZE} ;
...
//-------------------------------------------------------------------------------------------------------------
bool WaveFileHeader::processHeader(IWaveFile::WaveDataIter& dataStart, IWaveFile::WaveDataIter dataEnd)
{

	// Assumes that the header data will be complete in this vector. There may be extra data after the header
	// but the full header will be present
...
//-------------------------------------------------------------------------------------------------------------
IWaveFile::WaveType WaveFileHeader::getType() const
{
	return mType ;
}
...
//-------------------------------------------------------------------------------------------------------------
bool WaveFileHeader::processBlockHeader(IWaveFile::WaveDataIter& dataStart, IWaveFile::WaveDataIter dataEnd)
{
...
//-------------------------------------------------------------------------------------------------------------
IWaveFile::WaveBlockType WaveFileHeader::getBlockType() const
{
	return mBlockType ;
}
...
//-------------------------------------------------------------------------------------------------------------
unsigned WaveFileHeader::getBlockLength() const
{
	return mBlockLength ;
}

//-------------------------------------------------------------------------------------------------------------
std::string WaveFileHeader::getError()
{
	std::string error("") ;
...
//-------------------------------------------------------------------------------------------------------------
unsigned Mplane::WaveFileHeader::getHeaderSize() const
{
	return HEAD_BYTES ;
}

//-------------------------------------------------------------------------------------------------------------
unsigned Mplane::WaveFileHeader::getBlockHeaderSize() const
{
...
//-------------------------------------------------------------------------------------------------------------
bool WaveFileHeader::createHeader(std::vector<uint8_t>& data, IWaveFile::WaveType type, unsigned headerLenDelta)
{
...
//-------------------------------------------------------------------------------------------------------------
bool WaveFileHeader::createBlockHeader(std::vector<uint8_t>& data, IWaveFile::WaveBlockType type,
		unsigned blockLen)
{
	ByteTransfer::appendByte(data, (uint8_t)type) ;
...
//-------------------------------------------------------------------------------------------------------------
void WaveFileHeader::setError(const std::string& error)
{
	if (!mError.empty())
		mError += "\n" ;
	mError += error ;
...
//-------------------------------------------------------------------------------------------------------------
bool WaveFileHeader::checkType(uint8_t type)
{
...
//-------------------------------------------------------------------------------------------------------------
bool WaveFileHeader::checkBlockType(uint8_t blockType)
{
	// TODO
	return true ;
}
...
//-------------------------------------------------------------------------------------------------------------
void WaveFileHeader::skipHeader(IWaveFile::WaveDataIter& dataStart) const
{
