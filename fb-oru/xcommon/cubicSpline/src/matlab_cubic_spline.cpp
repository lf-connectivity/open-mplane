...
void matlab_cubic_spline(const emxArray_real_T *x, const emxArray_real_T *y,
  const emxArray_real_T *xx, emxArray_real_T *yy)
{
  ...
  /*  This function is simply a wrapper for the Matlab built-in cubic spline function. It has been written */
  /*  in order that we can transcode this function to C++, using the Matlab Coder toolbox */
  ...
  for (ix = 0; ix < nx; ix++) {
    if (rtIsNaN(xx->data[ix])) {
      ...
      for (high_i = 2; high_i <= numTerms; high_i++) {
        v = xloc * v + pp_coefs->data[(low_i + (high_i - 1) * (pp_breaks->size[1]
          - 1)) - 1];
      }
    }
