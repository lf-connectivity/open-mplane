...
public:
	...
	/**
	 * Factory for making key objects using the board serial number, hostid, and the GUI hostid.
	 */
	static std::shared_ptr<IKey> factory(const std::string& serialNum, const std::string& hostid, const std::string& ctrlHostid) ;

	/**
	 * Get the application-specific singleton. This will be provided by the embedded application (normally within a wrapper that stores the
	 * information to flash)
	 */
	static std::shared_ptr<IKey> getApplicationInstance() ;

	/**
	 * Get the global singleton. This will be provided by the embedded application (normally within a wrapper that stores the
	 * information to flash). The global key uses a hard-coded public key string that is shared among all applications
	 */
	...
	/**
	 * Set the license key from the board serial number, hostid, and the GUI hostid.
	 ...
	 */
	virtual bool setLicenseKey(const std::string& serialNum, const std::string& hostid, const std::string& ctrlHostid) =0 ;
	...
	/**
	 * Takes in 32-bits, expands the value to 128-bits, then encodes that value and returns a hex string
	 * @param bits
	 * @return hex string
	 */
	 ...
	 */
	virtual bool decode32bits(const std::string& encoded32bits, uint32_t& decoded32bits) const =0 ;
