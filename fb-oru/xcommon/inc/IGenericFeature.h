...
public:
	 ...
	 */
	static std::shared_ptr<IGenericFeature> factory(const std::vector<std::string>& featureNames,
			std::shared_ptr<IKey> key, GenericFeatureType features) ;
...
operator&(IGenericFeature::GenericFeatureType __a, IGenericFeature::GenericFeatureType __b)
{ return IGenericFeature::GenericFeatureType(static_cast<int>(__a) & static_cast<int>(__b)); }
...
#define GENERIC_FEATURE(val)	static_cast<IGenericFeature::GenericFeatureType>(val)
