...
namespace {
	...
	inline uint32_t leftrotate(uint32_t value, unsigned int steps)
	{
...
//-------------------------------------------------------------------------------------------------------------
std::vector<uint8_t> Sha1::getHash(const std::vector<uint8_t>& data)
{
	//@@	Initialize variables:
	//@@
	//@@	h0 = 0x67452301
	...
	// Buffer for the 80 32-bit words (derived from each block of sizteen 32-bit words)
	uint32_t w[80] ;
...
//	std::cerr << "[NEW] endCurrentBlock=" << endCurrentChunk << " bytelength=" << data.size() << " : currentBlock=" << currentChunk << std::endl ;
...
//		std::endl ;
        ...
        w[remainingBytes / 4] |= (uint32_t) data[remainingBytes + currentChunk] << ((3 - (remainingBytes % 4)) * 8);
        ++remainingBytes ;
    }

//std::cerr << "[NEW]  * w[" << (remainingBytes / 4) << "] |= 0x80 << " << (((3 - (remainingBytes & 3)) << 3)) << std::endl ;
...
//-------------------------------------------------------------------------------------------------------------
std::vector<uint8_t> Sha1::getHash(const std::string& data)
{
...
//-------------------------------------------------------------------------------------------------------------
std::string Sha1::getHashString(const std::vector<uint8_t>& data)
{
	return hashToString( getHash(data) ) ;
}

//-------------------------------------------------------------------------------------------------------------
std::string Sha1::getHashString(const std::string& data)
{
	return hashToString( getHash(data) ) ;
}

//-------------------------------------------------------------------------------------------------------------
std::string Sha1::hashToString(const std::vector<uint8_t>& data)
{
	std::stringstream ss ;
...
//-------------------------------------------------------------------------------------------------------------
void Sha1::calcHash(uint32_t h[], uint32_t w[])
{
	//@@		Initialize hash value for this chunk:
	//@@		a = h0
	//@@		b = h1
