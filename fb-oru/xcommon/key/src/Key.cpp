...
// Mplane Proprietary
= {
		,
}
// Mplane Proprietary
;
...
// Mplane Proprietary
#endif
		6, 13, 1, 14, 10, 19, 2, 7, 12, 0, 4, 16, 3, 5, 17, 18, 15, 9, 7, 11
...
//-------------------------------------------------------------------------------------------------------------
std::shared_ptr<IKey> IKey::factory()
{
	return std::make_shared<Key>() ;
}

//-------------------------------------------------------------------------------------------------------------
std::shared_ptr<IKey> IKey::factory(const std::string& serialNum, const std::string& hostid, const std::string& ctrlHostid)
{
...
//-------------------------------------------------------------------------------------------------------------
void Key::setKeyEncodeType(KeyEncodeType type)
{
	Key::mGlobalKeyType = type ;
}

//-------------------------------------------------------------------------------------------------------------
void IKey::setKeyEncodeType(KeyEncodeType type)
{
...
//-------------------------------------------------------------------------------------------------------------
bool Key::setLicenseKey(const std::string& licenseKey)
{
	uint32_t key[4] ;
...
//-------------------------------------------------------------------------------------------------------------
bool Key::setLicenseKey(const std::vector<std::string>& info)
{
...
#ifdef DEBUG_KEY
	std::cerr << "INFO '" << infoStr << "' SHA1 '" << sha1 << "' KEY '" << licenseKey << "'" << std::endl ;
#endif
...
//-------------------------------------------------------------------------------------------------------------
bool Key::setLicenseKey(const std::string& serialNum, const std::string& hostid, const std::string& ctrlHostid)
{
	std::vector<std::string> info ;
	info.push_back(serialNum) ;
	info.push_back(hostid) ;
// Mplane Proprietary
// Make it look like we use this string in the open source code, when in fact we don't
#if 0
// Mplane Proprietary
...
//-------------------------------------------------------------------------------------------------------------
bool Key::setFromEncodedKey(const std::string& encodedKey)
{
	uint32_t encoded[4] ;
...
//-------------------------------------------------------------------------------------------------------------
std::vector<uint32_t> Key::licenseKey() const
{
	return {mLicenseKey[0], mLicenseKey[1], mLicenseKey[2], mLicenseKey[3]} ;
}

//-------------------------------------------------------------------------------------------------------------
bool Key::isSet() const
{
	return mKeySet ;
}

//-------------------------------------------------------------------------------------------------------------
std::string Key::encode32bits(uint32_t bits) const
{
	...
	// Mplane Proprietary
	// Use the table to randomly distribute each of the 32-bits to 4 bit positions within the 128-bit result
	// Mplane Proprietary
	uint32_t encode[4] ;
...
//-------------------------------------------------------------------------------------------------------------
bool Key::decode32bits(const std::string& encoded32bits, uint32_t& decoded32bits) const
{
	decoded32bits = 0 ;
...
#ifdef DEBUG_KEY
std::cerr << "decode 0x" << encoded32bits << " 0x" << std::hex << decoded32bits << std::dec << std::endl ;
#endif
...
//-------------------------------------------------------------------------------------------------------------
bool Key::stringToUints(const std::string& str, uint32_t value[4]) const
{
	if (str.size() != 32)
		return false ;
	...
	{
		value[block] = static_cast<uint32_t>(strtoul( str.substr((3-block)*8, 8).c_str(), nullptr, 16 )) ;
	}
	return true ;
}

//-------------------------------------------------------------------------------------------------------------
std::string Key::uintsToString(uint32_t value[4]) const
{
	std::stringstream ss ;
...
//-------------------------------------------------------------------------------------------------------------
void Key::copyKey(uint32_t src[4], uint32_t dest[4]) const
{
	dest[0] = src[0] ;
	dest[1] = src[1] ;
	dest[2] = src[2] ;
...
//-------------------------------------------------------------------------------------------------------------
void Key::expand32(uint32_t src, uint32_t dest[4]) const
{
	dest[0] = 0 ;
	dest[1] = 0 ;
	dest[2] = 0 ;
...
#ifdef DEBUG_KEY
	std::cerr << "expand32(0x" << std::hex << src << ") replication=" << static_cast<int>(mKeyType) << std::endl ;
#endif
...
#ifdef VERBOSE_DEBUG_KEY
			std::cerr << std::dec << " * Bit " << bit << " Block " << block << " Idx " << idx <<
					" : pos " << (unsigned)bitpos <<
					" word num " << (unsigned)wordNum <<
					" word bit " << (unsigned)wordBit <<
					" : Actual bit " << ((src & mask) ? "1" : "0") <<
					"  final bit " << (bitVal ? "1" : "0") <<
					" : dest[" << (unsigned)wordNum << "]=0x" << std::hex << dest[wordNum] <<
					std::dec <<
					std::endl ;
#endif
		}
...
#ifdef DEBUG_KEY
	std::cerr << "expand32(0x" << std::hex << src << ") = " << uintsToString(dest) << std::endl ;
#endif
}

//-------------------------------------------------------------------------------------------------------------
bool Key::compress32(uint32_t src[4], uint32_t& decoded32bits) const
{
#ifdef DEBUG_KEY
	std::cerr << "compress32() replication=" << static_cast<int>(mKeyType) << std::endl ;
#endif

	decoded32bits = 0 ;

	// Mplane Proprietary
	// Use the table to find each of the four copies of the 32-bits and make sure all four copies are the same
	// Mplane Proprietary
	uint32_t mask{1} ;
	int idx(0) ;
	for (unsigned bit=0; bit < 32; ++bit, mask <<= 1)
...
#ifdef VERBOSE_DEBUG_KEY
			std::cerr << std::dec << " * * Bit " << bit <<
					" : mask=0x" << std::hex << mask <<
					" : decoded32bits=0x" << std::hex << decoded32bits <<
					std::dec <<
					std::endl ;
#endif
...
#ifdef VERBOSE_DEBUG_KEY
				std::cerr << std::dec << "ERROR not all bits set - invalid encryption" << std::endl ;
#endif
				return false ;
			}
		}
