 ...
 *
 *
 * \details   Takes in an external 128-bit license key and combines this with an private shared key so that two or more applications can share the
 * 				license key by passing around an encoded key
 *
 */
...
public:
	 ...
	 */
	virtual bool setLicenseKey(const std::string& serialNum, const std::string& hostid, const std::string& ctrlHostid) override ;
	 ...
	 */
	virtual bool decode32bits(const std::string& encoded32bits, uint32_t& decoded32bits) const override ;
