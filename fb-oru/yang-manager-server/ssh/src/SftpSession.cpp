...
//-------------------------------------------------------------------------------------------------------------
/**
 * NOTE: Whatever you do, do NOT allocate a buffer of this size on the stack. Because the stack is running
 * inside a thread (where the default stack size is small) you'll end up with really weird behaviour and segfaults
 * as the stack overflows!
 *
 */
...
public:
	...
	void show() const
	{
		...
		case SFTP_UPLOAD_FILE:
			std::cerr << "UPLOAD FILE remote=" << mRemotePath << " local=" << mLocalPath ;
			break ;

		case SFTP_DOWNLOAD_FILE:
			std::cerr << "DOWNLOAD FILE remote=" << mRemotePath << " local=" << mLocalPath ;
			break ;
...
//-------------------------------------------------------------------------------------------------------------
SftpSession::SftpSession(std::shared_ptr<SshSession> ssh) :
	...
	mDataBuffer( (char *)calloc(BUFF_LEN, sizeof(char)) )
{
    ...
    if (mSession == nullptr)
    {
		setError("Error allocating SFTP session: " + std::string(ssh_get_error(mSsh->session()))) ;
		return ;
    }
    ...
    if (rc != SSH_OK)
    {
		setError("Error initialising SFTP session: " + std::string(ssh_get_error(mSsh->session()))) ;
		return ;
    }
...
//-------------------------------------------------------------------------------------------------------------
void SftpSession::setDebug(unsigned level)
{
	mSsh->setDebug(level) ;
}

//-------------------------------------------------------------------------------------------------------------
std::string SftpSession::error()
{
	std::string tmp ;
	swap(mError, tmp) ;
	return tmp ;
...
//-------------------------------------------------------------------------------------------------------------
bool SftpSession::isConnected() const
{
	return mConnected ;
}

//-------------------------------------------------------------------------------------------------------------
std::string SftpSession::user() const
{
	return mSsh->user() ;
}

//-------------------------------------------------------------------------------------------------------------
std::string SftpSession::host() const
{
	return mSsh->host() ;
}

//-------------------------------------------------------------------------------------------------------------
bool SftpSession::fileUpload(const std::string &remoteFile,
		const std::string &localFile, ISftpSession::SftpFileCallback complete,
...
//-------------------------------------------------------------------------------------------------------------
bool SftpSession::fileDownload(const std::string &remoteFile,
		const std::string &localFile, ISftpSession::SftpFileCallback complete,
		unsigned timeoutSecs)
{
	if (!mConnected)
...
//-------------------------------------------------------------------------------------------------------------
bool SftpSession::dataUpload(const std::string &remoteFile,
		const std::string &data, ISftpSession::SftpDataCallback complete,
		unsigned timeoutSecs, int fileOptions)
{
	if (!mConnected)
	{
		setError("SFTP session not created") ;
...
//-------------------------------------------------------------------------------------------------------------
bool SftpSession::dataDownload(const std::string &remoteFile,
...
//-------------------------------------------------------------------------------------------------------------
void SftpSession::setError(const std::string &error)
{
	if (!mError.empty())
		mError += "\n" ;
	mError += error ;
...
//-------------------------------------------------------------------------------------------------------------
std::string SftpSession::sshError(const std::string &msg)
{
	return std::string(msg) + std::string(ssh_get_error(mSsh->session())) ;
}

//-------------------------------------------------------------------------------------------------------------
void SftpSession::addMkdir(const std::string remotePath)
{
...
//-------------------------------------------------------------------------------------------------------------
void SftpSession::handleMkdir(std::shared_ptr<Job> job)
{
#ifdef DEBUG_SFTP
...
//	job->mFileCallback(job->mRemotePath, "", SFTP_UPLOAD_FILE, job->mError, false) ;

}

//-------------------------------------------------------------------------------------------------------------
void SftpSession::handleFileUpload(std::shared_ptr<Job> job)
{
#ifdef DEBUG_SFTP
	std::cerr << __FUNCTION__ << std::endl ;
	job->show() ;
	...
	if (fd < 0)
	{
		...
		job->mFileCallback(job->mRemotePath, job->mLocalPath, SFTP_UPLOAD_FILE, job->mError, false) ;
		return ;
	}
	...
	job->mFileCallback(job->mRemotePath, job->mLocalPath, SFTP_UPLOAD_FILE, job->mError, timer->isTimedOut()) ;
}

//-------------------------------------------------------------------------------------------------------------
void SftpSession::handleFileDownload(std::shared_ptr<Job> job)
{
#ifdef DEBUG_SFTP
	std::cerr << __FUNCTION__ << std::endl ;
	job->show() ;
	...
	if (file == nullptr)
	{
		...
		job->mFileCallback(job->mRemotePath, job->mLocalPath, SFTP_DOWNLOAD_FILE, job->mError, false) ;
		return ;
	}
	...
	int fd = open(job->mLocalPath.c_str(), O_WRONLY | O_CREAT | O_TRUNC, stats->permissions) ;
	if (fd < 0)
	{
		std::string errorstr ;
		mDataBuffer[0] = '\0' ;
	...
	job->mFileCallback(job->mRemotePath, job->mLocalPath, SFTP_DOWNLOAD_FILE, job->mError, timer->isTimedOut()) ;
}

//-------------------------------------------------------------------------------------------------------------
void SftpSession::handleDataUpload(std::shared_ptr<Job> job)
{
#ifdef DEBUG_SFTP
	std::cerr << __FUNCTION__ << std::endl ;
	job->show() ;
	...
	doUpload(job->mRemotePath, job->mDataFileOptions, getData, job->mError, timer) ;
	job->mDataCallback(job->mRemotePath, "", SFTP_UPLOAD_DATA, job->mError, timer->isTimedOut()) ;
}

//-------------------------------------------------------------------------------------------------------------
void SftpSession::handleDataDownload(std::shared_ptr<Job> job)
{
#ifdef DEBUG_SFTP
	std::cerr << __FUNCTION__ << std::endl ;
	job->show() ;
...
#ifdef DEBUG_SFTP
		std::cerr << "handleDataDownload::::putData size=" << data.size() << std::endl ;
#endif
	...
	doDownload(job->mRemotePath, putData, job->mError, timer) ;
	job->mDataCallback(job->mRemotePath, job->mData, SFTP_DOWNLOAD_DATA, job->mError, timer->isTimedOut()) ;
}

//-------------------------------------------------------------------------------------------------------------
bool SftpSession::doUpload(const std::string &remotePath, int fileOptions, GetDataFunc getData, std::string &error,
		std::shared_ptr<TimeoutTimer> timer)
{
#ifdef DEBUG_SFTP
	...
	sftp_file file = sftp_open(mSession, remotePath.c_str(), O_WRONLY | O_CREAT | O_TRUNC, fileOptions);
	if (file == nullptr)
	{
...
//-------------------------------------------------------------------------------------------------------------
bool SftpSession::doDownload(const std::string &remotePath, PuttDataFunc putData, std::string &error,
		std::shared_ptr<TimeoutTimer> timer)
{
#ifdef DEBUG_SFTP
	std::cerr << __FUNCTION__ << " remote=" << remotePath << std::endl ;
