...
//-------------------------------------------------------------------------------------------------------------
std::shared_ptr<ISftpMgr> ISftpMgr::singleton()
{
...
//-------------------------------------------------------------------------------------------------------------
std::shared_ptr<ISftpSession> ISftpMgr::factory(
		const std::string &host, const std::string &user,
		const std::string &authData, AuthenticationType auth,
		const std::vector<ISshSession::PublicKey>& serverKeys,
		std::string &error)
{
	std::shared_ptr<SftpMgr> mgr(std::dynamic_pointer_cast<SftpMgr>(ISftpMgr::singleton())) ;
...
//-------------------------------------------------------------------------------------------------------------
void ISftpMgr::setDebug(unsigned level)
{
	std::shared_ptr<SftpMgr> mgr(std::dynamic_pointer_cast<SftpMgr>(ISftpMgr::singleton())) ;
...
//-------------------------------------------------------------------------------------------------------------
void ISftpMgr::disconnect(std::shared_ptr<ISftpSession>& session)
{
	std::shared_ptr<SftpMgr> mgr(std::dynamic_pointer_cast<SftpMgr>(ISftpMgr::singleton())) ;
...
//-------------------------------------------------------------------------------------------------------------
void ISftpMgr::hostKeyTypes(const std::string& typesList)
{
	std::shared_ptr<SftpMgr> mgr(std::dynamic_pointer_cast<SftpMgr>(ISftpMgr::singleton())) ;
...
//-------------------------------------------------------------------------------------------------------------
void SftpMgr::setDebugLevel(unsigned level)
{
	mDebug = level ;
	for (auto entry : mSessions)
	{
...
//-------------------------------------------------------------------------------------------------------------
void SftpMgr::disconnectSession(std::shared_ptr<ISftpSession>& sftp)
{
//std::cerr << "SftpMgr::disconnectSession count=" << sftp.use_count() << std::endl ;

	// See how many users of this pointer there are. If there are only 2 (i.e. the caller and this)
	// then we are safe to shut down the connection
	if (sftp.use_count() > 2)
		return ;
...
//-------------------------------------------------------------------------------------------------------------
std::shared_ptr<ISftpSession> SftpMgr::factoryCreate(
		const std::string &host, const std::string &user,
		const std::string &authData, ISftpMgr::AuthenticationType auth,
		const std::vector<ISshSession::PublicKey>& serverKeys,
		std::string &error)
{
	...
	// If we have server keys specified then use them to determine the valid list of server key types
	// we will accept
	if (!serverKeys.empty())
	{
...
//-------------------------------------------------------------------------------------------------------------
std::shared_ptr<ISftpSession> SftpMgr::factoryCreate(const std::string &remotePath,
		const std::string &authData, AuthenticationType auth,
		const std::vector<ISshSession::PublicKey>& serverKeys,
		std::string &error)
{
...
//-------------------------------------------------------------------------------------------------------------
void SftpMgr::setHostKeyTypes(const std::string &typesList)
{
