...
// ====================================================================================================
YangConfig::YangConfig() : Loggable("YangConfig", "YANG"),
...
// ====================================================================================================
std::string Mplane::YangConfig::getAccess(std::string path)
{
	std::string value = "\0";
	std::string access = "\0";
...
// ====================================================================================================
std::string YangConfig::getValue(std::string path)
{
	std::string value = "\0";
	std::string access = "\0";
...
// ====================================================================================================
void Mplane::YangConfig::commonGet(std::string path, std::string &value,
		std::string &access)
{
	...
	do
	{
		...
		commonSearch( elementSearch, pathCopy, foundListDelimiter, elementFound, type, indexStr, pathDelimiterPos );
		...
		if(nullptr != elementFound)
		{
			...
			else if( type == listLeafStr )
			{
				...
				if( true == foundListDelimiter )
				{
					...
					{
						// List leafs are a bit different. They have no leaf but have a entry with a value
						searchComplete = true;
...
// ====================================================================================================
bool YangConfig::isModuleSupported(std::string module)
{
...
// ====================================================================================================
void YangConfig::listSupportedModules(std::string & moduleList)
{
	mConfig->listElements(moduleStr, moduleList);
}

// ====================================================================================================
void YangConfig::listModuleFeatures(std::string module, std::map<std::string, bool> & featureList)
{
	std::shared_ptr<IYangElement> moduleElement = mConfig->getElement(moduleStr, module);
	...
	if (nullptr != moduleElement)
	{
		...
		for (auto &elem : moduleChildren)
		{
			...
			{
				featureList[elem->getName()] = (elem->getValue() == "true" ? true : false);
			}
		}
	}
}
...
// ====================================================================================================
void YangConfig::listModuleFeatures(std::string module, std::string & featureList)
{
	std::shared_ptr<IYangElement> moduleElement = mConfig->getElement(moduleStr, module);
...
// ====================================================================================================
bool YangConfig::isModuleFeatureEnabled(std::string module, std::string feature)
{
...
// ====================================================================================================
std::shared_ptr<IYangElement> YangConfig::getConfigFile(void)
{
	return mConfig;
}

// ====================================================================================================
std::string Mplane::YangConfig::getModuleVersion(std::string module)
{
	std::shared_ptr<IYangElement> found = mConfig->getElement(moduleStr, module);
...
std::string Mplane::YangConfig::getChoiceCaseName(std::string path)
{
	bool searchComplete = false;
	bool searchAbort = false;
	...
	do
	{
		...
		if(nullptr != elementFound)
		{
			...
			// This might not be the string we are looking for but lists are everywhere
...
// ====================================================================================================
unsigned int Mplane::YangConfig::getNumOfListEntries(std::string path)
{
	return commonGetNumEntries(path, listStr);
}

// ====================================================================================================
unsigned int Mplane::YangConfig::getNumOfListLeafEntries(std::string path)
{
	return commonGetNumEntries(path, listLeafStr);
}

// ====================================================================================================
void YangConfig::commonSearch( std::shared_ptr<IYangElement> elementSearch, std::string & path,
		bool & foundListDelimiter, std::shared_ptr<IYangElement> & elementFound, std::string & type,
		std::string & index, std::size_t & pathDelimiterPos)
{
	...
	if( 0 != pathDelimiterPos)
	{
		...
		if(0 != typeDelimiterPos)
		{
			...
			std::string name = path.substr(typeDelimiterPos + 1, (pathDelimiterPos - typeDelimiterPos) - 1);
...
// ====================================================================================================
bool Mplane::YangConfig::getConfigLoaded(void)
{
	return mIsConfigLoaded;
}

// ====================================================================================================
void Mplane::YangConfig::setConfigLoaded(bool configLoaded)
{
	mIsConfigLoaded = configLoaded;
}

// ====================================================================================================
void Mplane::YangConfig::listConfigTree(std::string &configTree)
{
	// We need our starting point in the configuration structure
...
// ====================================================================================================
void Mplane::YangConfig::recursiveElementSearch(std::shared_ptr<IYangElement> parentElement,
												 ...
												 unsigned int indent)
{
	...
	for(std::vector<std::shared_ptr<IYangElement>>::iterator iter = childElementVector.begin();
			iter != childElementVector.end(); iter ++ )
	{
		...
		resultString += elementType + " " + element->getName() + " "  + element->getValue() + " " + element->getVersion() + "\n";

		// Increase the indentation if this is not a leaf, a module or a list leaf entry.
		if( (leafStr != elementType) &&
			(moduleStr != elementType) &&
		...
		// For list leaf we have to handle it correctly, otherwise subsequent elements at that level
...
// ====================================================================================================
unsigned int Mplane::YangConfig::commonGetNumEntries(std::string path, std::string elementType)
{
	bool searchComplete = false;
	bool searchAbort = false;
	...
	do
	{
		...
		if(nullptr != elementFound)
		{
			...
			else if( type == listStr )
			{
				...
				{
					// We didn't find the list delimiter
					searchAbort = true;
				}
			}

			if( false == searchComplete )
			{
				// Found this part move on
				elementSearch = elementFound;
...
// ====================================================================================================
void Mplane::YangConfig::setYangVersion(std::string yangVersion)
{
	mYangVersion = yangVersion;
}

// ====================================================================================================
void Mplane::YangConfig::setOranVersion(std::string oranVersion)
{
	mOranVersion = oranVersion;
}

// ====================================================================================================
void Mplane::YangConfig::setProduct(std::string productName)
{
	mProductName = productName;
}

// ====================================================================================================
void Mplane::YangConfig::setVariant(std::string productVariant)
{
	mVarientName = productVariant;
}

// ====================================================================================================
void Mplane::YangConfig::setFormatVersion(std::string formatVersion)
{
	mFormatVersion = formatVersion;
}

// ====================================================================================================
void Mplane::YangConfig::setCompanyName(std::string companyName)
{
	mCompanyName = companyName;
}

// ====================================================================================================
std::string Mplane::YangConfig::getYangVersion(void)
{
	return mYangVersion;
}

// ====================================================================================================
std::string Mplane::YangConfig::getOranVersion(void)
{
	return mOranVersion;
}

// ====================================================================================================
std::string Mplane::YangConfig::getProduct(void)
{
	return mProductName;
}

// ====================================================================================================
std::string Mplane::YangConfig::getVariant(void)
{
	return mVarientName;
}

// ====================================================================================================
std::string Mplane::YangConfig::getFormatVersion(void)
{
	return mFormatVersion;
}

// ====================================================================================================
std::string Mplane::YangConfig::getCompanyName(void)
{
	return mCompanyName;
}
