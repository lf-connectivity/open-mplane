...
public:
	...
	HandlerCallback(YangHandlerSysrepoUnlogged* parent, GetPathsFunc roFunc, SetValFunc setValFunc, ChangeCompleteFunc completeFunc) ;
	 ...
	 */
	virtual int module_change(sysrepo::S_Session sess, const char *module_name, const char *xpath, sr_event_t event,
			uint32_t request_id, void *private_data) override ;

private:
	// Check for pseudo read-only
	int checkPseudoReadOnly(sysrepo::S_Session sess, const char *module_name, const std::string& xpathString) ;

	// Handle the change event - validate changes
	int handleChangeEvent(sysrepo::S_Session sess, const char *module_name, const std::string& xpathString) ;

	// handle the done event
	int handleDoneEvent(sysrepo::S_Session sess, const char *module_name, const std::string& xpathString);
