...
public:
	 ...
	 */
	static void measTypeToStr(IPerformanceMgmt::TransceiverMeasType_T type, std::string & str);
	static void measStrToType(std::string str, IPerformanceMgmt::TransceiverMeasType_T& type);

	/*
	 * Method to convert between transceiver measurement type enum and identifier
	 */
	static void measTypeToId(IPerformanceMgmt::TransceiverMeasType_T type, int & id);
	static void measIdToType(int id, IPerformanceMgmt::TransceiverMeasType_T& type);

	/*
	 * Method to convert between reporting info type enum and string
	 */
	static void reportTypeToStr(IPerformanceMgmt::TransReportInfoType_T type, std::string & str);
	static void reportStrToType(std::string str, IPerformanceMgmt::TransReportInfoType_T& type);

	/*
	 * Method to convert between measurement function type enum and string
	 */
	static void functionTypeToStr(IPerformanceMgmt::FunctionType_T type, std::string & str);
	static void functionStrToType(std::string str, IPerformanceMgmt::FunctionType_T& type);

	/*
	 * Method to convert between Rx window measurement type enum and string
	 */
	static void rxWindowMeasTypeToStr(IPerformanceMgmt::RxWindowMeasType_T type, std::string & str);
	static void rxWindowMeasStrToType(std::string str, IPerformanceMgmt::RxWindowMeasType_T& type);

	/*
	 * Method to convert between Rx window object unit id enum and string
	 */
	static void rxWindowObjIdToStr(IPerformanceMgmt::RxWindowObjectUnitId_T type, std::string & str);
	static void rxWindowObjIdStrToType(std::string str, IPerformanceMgmt::RxWindowObjectUnitId_T& type);

	/*
	 * Method to convert between transceiver measurement type enum and identifier
	 */
	static void rxWindowMeasTypeToId(IPerformanceMgmt::RxWindowMeasType_T type, int & id);
	static void rxWindowMeasIdToType(int id, IPerformanceMgmt::RxWindowMeasType_T& type);
	 ...
	 */
	virtual void setTransMeasActiveInactive(TransceiverMeasType_T measType, bool activate) =0 ;

	/**
	 * Set the function type for a transceiver measurement type
	 */
	virtual void setTransMeasFunction(TransceiverMeasType_T measType, FunctionType_T function) =0 ;

	/**
	 * Add/Remove the report type for a transceiver measurement type
	 */
	virtual void setTransMeasReportType(TransceiverMeasType_T measType, TransReportInfoType_T report, bool add) =0 ;

	/**
	 * Get the report type list for a transceiver measurement type
	 */
	virtual void getTransMeasReportType(TransceiverMeasType_T measType, std::set<TransReportInfoType_T>& report) =0 ;

	/**
	 * Set the bin count for the frequency table for a transceiver measurement type
	 */
	virtual void setTransMeasBinCount(TransceiverMeasType_T measType, uint16_t binCount) =0 ;
	 ...
	 */
	virtual void setTransMeasLowerLimit(TransceiverMeasType_T measType, double lowerLimit) =0 ;

	/**
	 * Set the upper limit for the frequency table for a transceiver measurement type
	 */
	virtual void setTransMeasUpperLimit(TransceiverMeasType_T measType, double upperLimit) =0 ;
	 ...
	 */
	virtual void setRxWindowMeasActiveInactive(RxWindowMeasType_T measType, bool activate) =0 ;

	/**
	 * Set the function type for the measurement
	 */
	virtual void setObjectUnitId(RxWindowMeasType_T measType, IPerformanceMgmt::RxWindowObjectUnitId_T id) =0 ;
	 ...
	 */
	virtual bool getRxWindowMeasResult(RxWindowMeasType_T measType, uint64_t& data) =0 ;
	 ...
	 */
	using NotificationCbFunc = std::function<void(std::map<TransceiverMeasType_T,std::vector<transMeasData>>,
			std::map<RxWindowMeasType_T,std::vector<rxWindowMeasData>>)> ;

	/**
	 * Configure notification for measurements
	 */
	virtual void configureNotification(uint16_t interval, NotificationCbFunc func) =0 ;

	/**
	 * Callback function with transceiver data for when the file upload interval expires
	 */
	using FileUploadCbFunc = std::function<void(std::map<TransceiverMeasType_T,std::vector<transMeasData>>,
			std::map<RxWindowMeasType_T,std::vector<rxWindowMeasData>>, std::string, std::string)> ;

	/**
	 * Configure file upload for measurements
	 */
	virtual void configureFileUpload(uint16_t interval, FileUploadCbFunc func) =0 ;
